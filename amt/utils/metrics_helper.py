...
def extract_pitches_intervals_from_notes(
    notes: List[Note], is_drum: bool = False
) -> Dict[str, np.ndarray]:
...
def extract_frame_time_freq_from_notes(
    notes: List[Note], is_drum: bool = False, hop_size_sec: float = 0.0625
) -> Dict[str, np.ndarray]:
    if len(notes) == 0:
        return {
            "time": np.array([]),
    ...
    if is_drum:
        frame_intervals = [
            [int(n.onset / hop_size_sec), int(n.onset / hop_size_sec) + 1]
            for n in notes
        ]
    else:
        frame_intervals = [
            [
                ...
                max(int(n.offset / hop_size_sec), int(n.onset / hop_size_sec) + 1),
            ]
            for n in notes
        ]
    ...
    return {
        ...
        "freqs": [
            np.array([midi_to_hz(p) for p in pitches]) for pitches in frame_pitches
        ],
        "roll": roll,
    }
...
def mir_eval_note_f1(
    ...
    if len(ref_pitches) == 0 and len(est_pitches) == 0:
        ...
        onset_p, onset_r, offset_p, offset_r = 0.0, np.nan, 0.0, np.nan
    # Add the following elif case to handle the situation when there are reference pitches but no estimated pitches
    elif len(ref_pitches) != 0 and len(est_pitches) == 0:
        metrics = {
            "onset_f": 0.0,  # No false positives, precision is NaN. recall and F1 are 0.
            "offset_f": 0.0,  # No false positives, precision is NaN. recall and F1 are 0.
        }
        onset_p, onset_r, offset_p, offset_r = np.nan, 0.0, np.nan, 0.0
    else:
        metrics = {}
        onset_p, onset_r, metrics["onset_f"], _ = precision_recall_f1_overlap(
            ref_intervals,
            ref_pitches,
            est_intervals,
            est_pitches,
        ...
        if is_drum is not True:
            offset_p, offset_r, metrics["offset_f"], _ = precision_recall_f1_overlap(
                ref_intervals,
                ref_pitches,
                est_intervals,
                est_pitches,
    ...
    if is_drum:
        ...
        metrics = {
            k + "_drum" if "onset" in k else k: v
...
# Evaluation: Frame F1
def mir_eval_frame_f1(
    est_time_freqs: Dict[str, List[np.ndarray]],
    ...
    # Add the following elif case to handle the situation when there are reference pitches but no estimated pitches
        ...
        metrics = {
            "frame_f": 0.0,  # F1-score will be 0.
            "frame_f_pc": 0.0,
        }
    else:
        # frame-wise evaluation
        res = evaluate(
            ref_time=ref_time_freqs["time"],
...
def mir_eval_melody_metric(
    ...
    try:
        ...
        logger.info('%s', f"Error occurred: {e}")
        return {
...
def test():
    ...
    ref_intervals = np.array([[0, 1], [2, 3], [5, 12], [1, 10]])  # in seconds
    ...
    est_pitches = np.array([101, 100, 200, 300])  # in Hz
    est_intervals = np.array(
        [[0, 0.5], [2, 3], [5, 12], [1, 10]]  # correct onset, on-offset incorrect
    )
    mir_eval_note_f1(est_pitches, est_intervals, ref_pitches, ref_intervals)
    ...
    np.append(
        est_pitches, 100
    )  # ref has 4 notes, while est has correct 4 notes + another 1 note.
