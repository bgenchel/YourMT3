...
from amt.utils.audio import load_audio_file, slice_padded_array
from amt.utils.tokenizer import EventTokenizerBase, NoteEventTokenizer
from amt.utils.note2event import slice_multiple_note_events_and_ties_to_bundle
from amt.utils.note_event_dataclasses import Note, NoteEvent, NoteEventListsBundle
...
class AudioFileDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[np.ndarray, Dict, NoteEventListsBundle]:
        ...
        note_events_dict = np.load(
            note_events_file, allow_pickle=True, fix_imports=False
        ).tolist()
        ...
        if self.task_manager.tokenizer is not None:
            ...
            token_array = self.task_manager.tokenize_note_events_batch(
                note_event_segments, start_time_to_zero=False, sort=True
            )
        ...
        return (
            torch.from_numpy(audio_segments),
            notes_dict,
            torch.from_numpy(token_array).long(),
        )
