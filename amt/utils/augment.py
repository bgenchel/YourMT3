...
from amt.utils.note_event_dataclasses import NoteEvent, NoteEventListsBundle
from amt.utils.note2event import (
    check_event_len_from_bundle,
    mix_note_event_lists_bundle,
    separate_by_subunit_programs_from_note_event_lists_bundle,
)
from amt.utils.utils import dict_iterator, extend_dict
...
def audio_random_submix_fn(
    ...
    processed_audio_mix = np.sum(
        processed_audio_stems, axis=1, keepdims=True
    )  # (b, 1, t)

    # Normalize
    if normalize is True:
        norm_factors = (
            np.max(np.abs(processed_audio_mix), axis=2, keepdims=True) + EPS
        )  # (b, 1, 1)
...
def audio_random_submix_processor(
    ...
    create_processed_audio_array: bool = True,
) -> None:
    ...
    if update_audio_segments is False and create_processed_audio_array is False:
        raise ValueError(
            "At least one of update_audio_segments and create_processed_audio_mix should be True."
        )
    ...
    if audio_masks is None:
        ...
        for i, audio_segment in enumerate(sampled_data["audio_segments"]):
            processed_audio_stems, processed_audio_mix = audio_random_submix_fn(
                x=audio_segment, random_amp_range=random_amp_range, mask=None
            )
            if create_processed_audio_array is True:
        ...
        for i, (audio_segment, mask) in enumerate(
            zip(sampled_data["audio_segments"], audio_masks)
        ):
            processed_audio_stems, processed_audio_mix = audio_random_submix_fn(
                x=audio_segment, random_amp_range=random_amp_range, mask=mask
            )
            if create_processed_audio_array is True:
                sampled_data["processed_audio_array"][i, :, :] = processed_audio_mix
            if update_audio_segments is True:
...
def drop_random_stems_from_bundle(
    sampled_data: Dict[str, Any], prob: float = 0.7
) -> None:
    """
    ...
    for i, (
        ...
        zip(
            sampled_data["has_stems_segments"],
        ...
        if has_stems is True and UNANNOTATED_PROGRAM not in programs:
            ...
            uniq_programs = np.unique(
                [
                    ne.program if not ne.is_drum else 128
                    for ne in (tie_note_events + note_events)
                ]
            )
            ...
            if (
                len(rand_sel_prgs) == 0 and len(uniq_programs) != 0
            ):  # Make sure at least one program is active
                rand_sel_prgs = np.random.choice(uniq_programs, size=1)
            programs_mask = np.isin(programs, rand_sel_prgs).astype(np.int32)
            drums_mask = (
                programs_mask * is_drum
            )  # NOTE: if drums are not annotated as program 128, this would not work properly
            ...
            note_events[:] = [
                ...
                if (not ne.is_drum and ne.program in _programs_in_use)
                or (ne.is_drum and _drum_in_use)
            ]
            tie_note_events[:] = [
                ne for ne in tie_note_events if ne.program in _programs_in_use
            ]
            ...
            if np.random.rand() > prob:
                ...
                assert (
                    UNANNOTATED_PROGRAM not in uniq_programs
                )  # 129 is not included here...
...
# -------------------------------------------------------------------------------------
def intra_stem_augment_processor(
    sampled_data: Dict[str, Any],
    random_amp_range: List[float] = [0.6, 1.2],
    prob: float = 0.7,
    update_audio_segments: bool = True,
    submix_audio: bool = True,
) -> None:
    """
...
# -------------------------------------------------------------------------------------
def combined_survival_and_stop(
    max_k: int = 5, tau: float = 0.3, alpha: float = 1.0
) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    prob_stop_at_k = -np.diff(np.append(survival, 0.0))
...
def deterministic_random_ux_sampler(prob_stop_at_k, bsz) -> np.ndarray:
    ...
    """
    ux_count_per_item = np.random.choice(
        np.arange(len(prob_stop_at_k)), size=bsz, p=prob_stop_at_k
    )
    return ux_count_per_item


def check_programs_overlap(
    list_programs: List[np.ndarray], programs: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def regroup_program_and_audio_by_minimal_shared_subunits(
    ...
    gathered_has_stem = [
        ...
        for programs, audio_array in zip(gathered_programs, gathered_audio_array)
    ]
    ...
    return (
        grouped_programs,
        grouped_prg2audio,
    )  # (List[Tuple[int]], DefaultDict[Tuple[int], List[int]])


def audio_random_submix_by_regroup_program_processor(
    gathered_programs: List[np.ndarray],
    gathered_audio_array: np.ndarray,
    submix_random_amp_range: List[float] = [0.9, 1.0],
    max_num_stems: int = 12,
    ...
    grouped_programs, grouped_prg2audio = (
        regroup_program_and_audio_by_minimal_shared_subunits(
    ...
    for i, prgs in enumerate(grouped_programs):
        audio_ids = grouped_prg2audio[
            prgs
        ]  # id of gathered_audio_array, e.g.:[(i,j),...]
        if len(audio_ids) == 1:
            # no need to submix, already subunits
            src_idx, stem_idx = audio_ids[0]
            submix_audio_array[:, i, :] = gathered_audio_array[src_idx][
                :, [stem_idx], :
            ]
        else:
            # submix audio from elements of subunit programs
            _submix_audio_list = [
                gathered_audio_array[src_idx][:, [stem_idx], :]
                for (src_idx, stem_idx) in audio_ids
            ]
            _submix_audio_arr = np.concatenate(
                _submix_audio_list, axis=1, dtype=np.float32
            )  # (1, C, T)
            _, _submix_audio_arr = audio_random_submix_fn(
                _submix_audio_arr,
...
def cross_stem_augment_processor(
    ...
    stem_xaug_policy: Dict = {
        ...
        "p_include_singing": 0.8,  # probability of including singing for cross augmented examples. if None, use base probaility.
    ...
    mix_audio: bool = True,
    create_subunit_note_events: bool = False,
) -> None:
    """
    ...
    x_hat = {
        ...
        "subunit_audio_array": np.zeros(
            (bsz, max_subunit_stems, n_frames), dtype=np.float32
        ),  # (B, max_submix_stems, T)
        ...
        "processed_audio_array": np.zeros(
            (bsz, 1, n_frames), dtype=np.float32
        ),  # mixed audio array, B, 1, T)
    }
    ...
    for i, (audio_array, ne_bundle, programs, has_unannotated) in enumerate(iter_x_in):
        ...
        if num_ux_samples > 0 and has_unannotated is False:
            ...
            gathered_ne_bundle = (
                ne_bundle  # mutable, but ok because `dict_iterator` yields new dict
            )
            gathered_audio_array = [audio_array]

            for k in range(num_ux_samples):
                # Get next external source
                ex_audio_array, ex_ne_bundle, ex_programs, ex_has_unannotated = next(
                    iter_ux
                )
                ...
                instr_overlap, uniq_ex_prg_mask = check_programs_overlap(
                    gathered_programs, ex_programs
                )
                ...
                if (
                    no_drum_overlap is True
                    and no_instr_overlap is False
                    and DRUM_PROGRAM in instr_overlap
                ):
                    non_drum_ex_prg_mask = np.array(
                        [prg != DRUM_PROGRAM for prg in ex_programs]
                    )
                ...
                if (
                    check_event_len_from_bundle(
                        gathered_ne_bundle, ex_ne_bundle, max_len=max_l
                    )
                    is False
                ):
                ...
                if ex_prg_mask is None:
                    ...
                    ex_ne_bundle["note_events"][0] = [
                        ne
                        for ne in ex_ne_bundle["note_events"][0]
                        if (not ne.is_drum and ne.program in ex_programs)
                        or (ne.is_drum and _ex_has_drum)
                    ]
                    ex_ne_bundle["tie_note_events"][0] = [
                        ne
                        for ne in ex_ne_bundle["tie_note_events"][0]
                        if ne.program in ex_programs
                    ]
                    extend_dict(gathered_ne_bundle, ex_ne_bundle)
            ...
            subunit_programs, subunit_audio_array = (
                audio_random_submix_by_regroup_program_processor(
                    gathered_programs,
                    gathered_audio_array,
                    max_num_stems=max_subunit_stems,
                )
            )
            mixed_ne_bundle = mix_note_event_lists_bundle(
                gathered_ne_bundle,
                sort=True,
            ...
            x_hat["subunit_audio_array"][
                i, : subunit_audio_array.shape[1], :
            ] = subunit_audio_array  # (B, C, T)
            ...
            if create_subunit_note_events is True:
                subunit_ne_bundle = (
                    separate_by_subunit_programs_from_note_event_lists_bundle(
                        ne_bundle, subunit_programs, start_time_to_zero=True, sort=True
                    )
                )
            else:
                subunit_ne_bundle = None
    ...
    if mix_audio is True:
        amp_applied_stem_arr, mix_audio_arr = audio_random_submix_fn(
            x_hat["subunit_audio_array"],
            random_amp_range=random_amp_range,
            mask=None,
            normalize=True,
