...
from amt.utils.note2event import validate_notes, trim_overlapping_notes
...
def find_channel_of_track_name(
    midi_file: os.PathLike, track_name_keywords: List[str]
) -> Optional[int]:
...
def midi2note(
    ...
    active_notes = [
        [] for i in range(16)
    ]  # active notes by channel(0~15). active_notes[ch] = [Note1, Note_2,..]
    sustained_notes = [
        [] for i in range(16)
    ...
    if quantize:
        for note in notes:
            note.onset = round(note.onset * 100) / 100.0
            note.offset = round(note.offset * 100) / 100.0
    ...
    if verbose > 0:
        logger.info('%s', f"parsed {file}: midi_type={midi.type}, num_notes={len(notes)}, num_instr={num_instr}, has_drum={has_drum}")
    if return_programs:
...
def note_event2midi(
    ...
    for prg in programs:
        if prg == 128:
            ...
            try:
                ...
                warnings.warn(
                    f"not available channels for program {prg}, share channel 16"
                )
    ...
    note_events.sort(
        key=lambda ne: (ne.time, ne.is_drum, ne.program, ne.velocity, ne.pitch)
    )
    ...
    for program in programs:
        ...
        if program == 128:
            ...
            program_name = output_inverse_vocab.get(
                program, (program, f"Prg. {str(program)}")
            )[1]
        else:
        ...
        if program == 128:  # drum
            ...
            track.append(
                Message(
                    "program_change",
                    program=singing_program_mapping,
                    time=0,
                    channel=channel,
                )
            )
            ...
            track.append(
                Message(
                    "program_change",
                    program=singing_chorus_program_mapping,
                    time=0,
                    channel=channel,
                )
            )
            ...
            track.append(
                Message("program_change", program=program, time=0, channel=channel)
            )
...
def pitch_shift_midi(
    ...
    src_note_events_file = os.path.join(
        src_midi_dir, f"{src_midi_filename}_note_events.npy"
    )
    ...
    for pitch_shift in range(min_pitch_shift, max_pitch_shift):
        ...
        # destination file
        dst_midi_file = os.path.join(
            src_midi_dir, f"{src_midi_filename}_pshift{pitch_shift}.mid"
        )
        dst_notes_file = os.path.join(
            src_midi_dir, f"{src_midi_filename}_pshift{pitch_shift}_notes.npy"
        )
        dst_note_events_file = os.path.join(
            src_midi_dir, f"{src_midi_filename}_pshift{pitch_shift}_note_events.npy"
        )
        ...
        if write_midi_file:
            ...
            logger.info('%s', f"Created {dst_midi_file}")
        ...
        if write_notes_file:
            ...
            np.save(
                dst_notes_file, dst_notes_metadata, allow_pickle=True, fix_imports=False
            )
            logger.info('%s', f"Created {dst_notes_file}")

        # write note events file
        if write_note_events_file:
            # get metadata for note events
            src_note_events_metadata = np.load(
                src_note_events_file, allow_pickle=True
            ).tolist()
            ...
            np.save(
                dst_note_events_file,
                dst_note_events_metadata,
                allow_pickle=True,
                fix_imports=False,
            )
            logger.info('%s', f"Created {dst_note_events_file}")
