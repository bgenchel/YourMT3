...
def discount_to_target(samples: np.ndarray, target_sum: int) -> np.ndarray:
    ...
    while samples.sum() > target_sum:
        ...
        if indices_to_discount.size == 0:
            ...
            logger.info('%s', "Cannot reach target sum without going below 1 for some elements.")
            return samples
...
def create_merged_train_dataset_info(
    data_preset_multi: dict, data_home: Optional[os.PathLike] = None
):
    ...
    for dp in data_preset_multi["presets"]:
        ...
        train_dataset_info["index_ranges"].append(
            (start_idx, start_idx + len(_file_list))
        )

    # set dataset weights
    if (
        "weights" in data_preset_multi.keys()
        and data_preset_multi["weights"] is not None
    ):
        train_dataset_info["dataset_weights"] = data_preset_multi["weights"]
        assert (
            len(train_dataset_info["dataset_weights"])
            == train_dataset_info["n_datasets"]
        )
    else:
        train_dataset_info["dataset_weights"] = np.ones(
            train_dataset_info["n_datasets"]
        )
        logger.info('%s', "No dataset weights specified, using equal weights for all datasets.")
    ...
    if len(dataset_weights) > 1 and len(dataset_index_ranges) > 1:
        for dataset_weight, index_range in zip(dataset_weights, dataset_index_ranges):
            ...
            sample_weight = dataset_weight * (
                1 - n_samples_in_dataset / n_total_samples_in_datasets
            )
...
def get_list_of_weighted_random_samplers(
    num_samplers: int,
    dataset_weights: List[float],
    dataset_index_ranges: List[Tuple[int]],
    num_samples_per_epoch: Optional[int] = None,
    replacement: bool = True,
) -> List[torch.utils.data.sampler.Sampler]:
