...
from amt.utils.note2event import note2note_event
from amt.utils.midi import note_event2midi
from amt.utils.note2event import slice_multiple_note_events_and_ties_to_bundle
...
from amt.utils.tokenizer import EventTokenizer, NoteEventTokenizer
from amt.utils.note_event_dataclasses import Note, NoteEvent, Event
from amt.config.vocabulary import GM_INSTR_FULL, GM_INSTR_CLASS_PLUS
...
def download_and_extract(
    ...
    if response.status_code != 200:
        logger.info('%s', f"Failed to download file. Status code: {response.status_code}")
        return
    ...
    logger.info('%s', f"Checksum (md5): {_check_sum}")

    if check_sum is not None and check_sum != _check_sum:
        raise ValueError(
            f"Checksum doesn't match! Expected: {check_sum}, Actual: {_check_sum}"
        )
...
def write_model_output_as_midi(
    notes: List[Note],
            ...
            if note.is_drum:
                ...
                new_notes.append(
                    Note(
                        ...
                        program=output_inverse_vocab.get(note.program, [note.program])[
                            0
                        ],
                        onset=note.onset,
                        offset=note.offset,
...
class Timer:
    ...
    def print_elapsed_time(self, message: Optional[str] = None) -> float:
        ...
        if message is not None:
            ...
            text = (
                f"⏰ elapse time: {int(minutes)}m {int(seconds)}s {milliseconds:.2f}ms"
            )
        logger.info('%s', text)
...
def merge_splits(
    splits: List[str], dataset_name: Union[str, List[str]]
) -> Dict[int, Any]:
    """
    merge_splits:
    ...
    if n_splits > 1 and isinstance(dataset_name, str):
        ...
        n_splits > 1
        and isinstance(dataset_name, list)
        and len(dataset_name) != n_splits
    ):
        raise ValueError(
            "The number of dataset names in list must be equal to the number of splits."
        )
    else:
        pass
...
def remove_ids_from_file_list(
    file_list: Dict[str, Any], selected_ids: List[int], reindex: bool = True
) -> Dict[int, Any]:
...
def deduplicate_splits(
    ...
    if isinstance(split_a, str):
        json_file_a = (
            f"{data_home}/yourmt3_indexes/{dataset_name}_{split_a}_file_list.json"
        )
    ...
    if isinstance(split_b, str):
        json_file_b = (
            f"{data_home}/yourmt3_indexes/{dataset_name}_{split_b}_file_list.json"
        )
    ...
    filtered_file_list_a = remove_ids_from_file_list(
        file_list_a, ids_to_remove, reindex
    )
...
def assert_note_events_almost_equal(
    actual_note_events,
    ...
    for j, (actual_note_event, predicted_note_event) in enumerate(
        zip(actual_note_events, predicted_note_events)
    ):
...
def note_event2token2note_event_sanity_check(
    note_events: List[NoteEvent], notes: List[Note], report_err_cnt=False
) -> Counter:
    # slice note events
    ...
    cnt_org_empty = 0
    cnt_recon_empty = 0
    for i, (recon_note_events, recon_tie_note_events, _, _) in enumerate(
    ...
    recon_notes, err_cnt = merge_zipped_note_events_and_ties_to_notes(
        zipped_note_events_and_tie, fix_offset=False
    )
