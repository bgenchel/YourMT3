...
from amt.utils.utils import note_event2token2note_event_sanity_check, freq_to_midi
...
def delete_hidden_files(base_dir):
    for hidden_file in glob.glob(os.path.join(base_dir, "**/.*"), recursive=True):
        ...
        logger.info('%s', f"Deleted: {hidden_file}")
...
def convert_annotation_to_notes(id, program, ann_files):
    ...
    for ann_file, prog in zip(ann_files, program):
        ...
        notes = mix_notes(
            [notes, notes_by_instr], sort=True, trim_overlap=True, fix_offset=True
        )
...
def data_bug_fix(base_dir):
    ...
    for file in files:
        ...
        logger.info('%s', f"Renamed: {file} -> {new_file}")
...
def preprocess_urmp16k(
    ...
    for dir_name in sorted(os.listdir(base_dir)):
        ...
        audio_tracks = []
        for i, p in enumerate(program_strings):
            audio_sep_file = os.path.join(
                base_dir, dir_name, f"AuSep_{i+1}_{p}_{id}_{title}.wav"
            )
            audio_track = (
                load_audio_file(audio_sep_file, dtype=np.int16) / 2**15
            )  # returns bytes
