            ...
            if dur < 0.01:
                ...
                if fix_offset is True:
                    if dist_to_next_onset < max_dur:
                        ...
                        logger.info('%s', f"Corrected track {i}: {v_old} to {ann_all[str(i)][j]}")

                else:
                    logger.info('%s %s %s', v, ann_all[str(i)][j + 1], f"dist_to_next_onset: {dist_to_next_onset}")


def preprocess_cmedia_16k(
    ...
    for id in ids_all:
        ...
        notes, note_events = create_note_note_event_midi_from_cmedia_annotation(
            ann, midi_file, id
        )
        ...
        if sanity_check:
            ...
            total_err += err_cnt
    if sanity_check:
        logger.info('%s', total_err)
        if sum(total_err.values()) > 0:
            raise Exception(
                "Sanity check failed. Please check the error messages above."
            )
        else:
            logger.info('%s', "Sanity check passed.")
