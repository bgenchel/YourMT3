...
from amt.utils.note2event import note2note_event, mix_notes
import mirdata
...
def create_audio_stem_from_mtrack(
    ds: mirdata.core.Dataset, mtrack_id: str, delete_source_files: bool = False
) -> Dict:
    ...
    for track_id in track_ids:
        ...
        if delete_source_files:
            logger.info('%s', f"🗑️ Deleting {audio_file} ...")
...
def create_note_event_and_note_from_mtrack_mirdata(
    ds: mirdata.core.Dataset, mtrack_id: str, fix_bass_octave: bool = True
) -> Tuple[Dict, Dict]:
    ...
    for track_id in track_ids:
        ...
        if fix_bass_octave == True and track.program_number in np.arange(32, 40):
            if track.plugin_name == "scarbee_jay_bass_slap_both.nkm":
                ...
                for note in notes:
                    note.pitch -= 12
                logger.info('%s %s', "Fixed bass octave for track", track_id)
...
def preprocess_slakh16k(
    data_home: str,
    run_checksum: bool = False,
    delete_source_files: bool = False,
    fix_bass_octave: bool = True,
) -> None:
    """
    ...
    if run_checksum:
        logger.info('%s', "Checksum for slakh dataset...")
        ds.validate()
    logger.info('%s', "Preprocessing slakh dataset...")
    ...
    for split in ["train", "validation", "test"]:
        ...
        for i, mtrack_id in enumerate(mtrack_ids):
            logger.info('%s', f"🏃🏻‍♂️: processing {mtrack_id} ({i+1}/{len(mtrack_ids)} in {split})")
            mtrack = ds.multitrack(mtrack_id)
            output_dir = os.path.dirname(mtrack.mix_path)  # same as mtrack
            """Audio: get stems (as array) and metadata from the multitrack"""
            stem_content = create_audio_stem_from_mtrack(
                ds, mtrack_id, delete_source_files
            )
        ...
        logger.info('%s', f"💾 Created {summary_file}")

        elapsed_time = time.time() - start_time
        logger.info('%s', f"⏰: {int(elapsed_time // 3600):02d}h {int(elapsed_time % 3600 // 60):02d}m {elapsed_time % 60:.2f}s")
...
def add_program_and_is_drum_info_to_file_list(data_home: str):
    ...
    for split in ["train", "validation", "test"]:
        ...
        with open(file, "w") as f:
            json.dump(file_list, f, indent=4)
        logger.info('%s', f"💾 Added program and drum info to {file}")
