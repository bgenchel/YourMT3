...
import numpy as np
from amt.utils.audio import get_audio_file_info, load_audio_file
from amt.utils.midi import midi2note, note_event2midi
...
from amt.utils.event2note import event2note_event
from amt.utils.note_event_dataclasses import Note, NoteEvent
from amt.utils.utils import note_event2token2note_event_sanity_check

logger = logging.getLogger(__name__)

SINGING_WITH_UNANNOTATED_PROGRAM = [
    100,
    129,
]  # 100 for singing voice, 129 for unannotated
...
def check_file_existence(file: str) -> bool:
    ...
    if not os.path.exists(file):
        ...
        logger.info('%s', f"File {file} is too short.")
        res = False
    return res


def create_spleeter_audio_stem(
    vocal_audio_file, accomp_audio_file, mir_st500_id
) -> Dict:
    ...
    vocal_audio = (
        load_audio_file(vocal_audio_file, dtype=np.int16) / 2**15
    )  # returns bytes
            ...
            if dur < 0.01:
                ...
                if fix_offset is True:
                    if dist_to_next_onset < max_dur:
                        ...
                        logger.info('%s', f"Corrected track {i}: {v} to {ann_all[str(i)][j]}")
                else:
                    logger.info('%s %s %s', v, ann_all[str(i)][j + 1], f"dist_to_next_onset: {dist_to_next_onset}")


def preprocess_mir_st500_16k(
    ...
    for i in range(1, 501):
        ...
        if (
            check_file_existence(audio_file)
            and check_file_existence(audio_vocal_file)
            and check_file_existence(audio_acc_file)
        ):
            audio_all[str(i)] = audio_file
        else:
            audio_missing[split].append(i)
    logger.info('%s', f'Number of missing audio files: train = {len(audio_missing["train"])}, test = {len(audio_missing["test"])}')
    ...
    for id in ids_all:
        ...
        if sanity_check:
            # sanity check
            logger.info('%s', f"Sanity check for {id}...")
            note_event2token2note_event_sanity_check(
                note_events["note_events"], notes["notes"]
            )
