...
from amt.utils.note2event import (
    ...
    extract_notes_selected_by_programs,
)
...
from amt.config.vocabulary import MT3_FULL_PLUS
...
def find_index_with_key(lst, key):
    ...
    indices = [
        ...
        if filtered_key in filter_string(value.lower())
    ]
...
def preprocess_geerdes16k(
    data_home=os.PathLike, dataset_name="geerdes", sanity_check=False
) -> None:
    """Preprocess Geerdes dataset."""
    # Directory and file paths
    base_dir = os.path.join(data_home, dataset_name + "_yourmt3_16k")
    ...
    for geerdes_id, v in tracks_all.items():
        ...
        note_events_file = os.path.join(
            note_processed_dir, geerdes_id + "_note_events.npy"
        )
        ...
        notes_voc["is_drum"] = [
            1 if p == DRUM_PROGRAM else 0 for p in notes_voc["program"]
        ]
        ...
        note_events_voc_file = os.path.join(
            note_processed_dir, geerdes_id + "_note_events_voc.npy"
        )
        np.save(
            note_events_voc_file, note_events_voc, allow_pickle=True, fix_imports=False
        )

        notes_acc = deepcopy(notes)
        notes_acc["notes"] = extract_notes_selected_by_programs(
            notes["notes"],
            [
                p
                for p in notes["program"]
        ...
        notes_acc["is_drum"] = [
            1 if p == DRUM_PROGRAM else 0 for p in notes_acc["program"]
        ]
        ...
        note_events_acc_file = os.path.join(
            note_processed_dir, geerdes_id + "_note_events_acc.npy"
        )
        np.save(
            note_events_acc_file, note_events_acc, allow_pickle=True, fix_imports=False
        )
    ...
    for task_suffix in task_suffixes:
        for split in splits:
            ...
            file_list_file = os.path.join(
                output_index_dir, f"{dataset_name}_{split}{task_suffix}_file_list.json"
            )
