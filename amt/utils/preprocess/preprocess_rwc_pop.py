...
def create_note_event_and_note_from_midi(
    mid_file: str, id: str, ignore_pedal: bool = True
) -> Tuple[Dict, Dict]:
...
def preprocess_rwc_pop16k(data_home=os.PathLike, dataset_name="rwc_pop") -> None:
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            ...
            mid_file = os.path.join(
                base_dir, "MIDI-Bass-Octave-fixed-v2", id + "_bass.mid"
            )
            # assert os.path.exists(mid_file)
            if not os.path.exists(mid_file):
                logger.info('%s %s', mid_file, "does not exist")
                continue
    ...
    for id in rwc_bass.keys():
        ...
        notes, note_events = create_note_event_and_note_from_midi(
            midi_file, id, ignore_pedal=True
        )
        ...
        logger.info('%s', f"Created {bpm120_midi_file}")
