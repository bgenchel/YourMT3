...
def preprocess_maestro16k(
    ...
    for i in ids_all:
        ...
        # Rename '.midi' to '.mid'
        if ".midi" in m["midi_filename"] and not os.path.exists(
            m["midi_filename"].replace(".midi", ".mid")
        ):
    ...
    for i in ids["all"]:
        ...
        if sanity_check:
            # sanity check
            logger.info('%s', f"Sanity check for {i}: {midi_file}")
