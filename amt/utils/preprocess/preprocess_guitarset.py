...
import numpy as np
import jams
from amt.utils.note_event_dataclasses import Note, NoteEvent
from amt.utils.audio import get_audio_file_info, pitch_shift_audio
from amt.utils.midi import note_event2midi, pitch_shift_midi
...
def generate_pitch_shifted_wav_and_midi(
    file_list: Dict, min_pitch_shift: int = -5, max_pitch_shift: int = 6
):
...
def preprocess_guitarset16k(
    data_home: os.PathLike,
    dataset_name: str = "guitarset",
    pitch_shift_range: Optional[Tuple[int, int]] = (-5, 6),
) -> None:
    """
    Splits:
    ...
    # Process annotations
    all_ann_files = glob.glob(
        os.path.join(base_dir, "annotation/*.jams"), recursive=True
    )
    ...
    for ann_file in all_ann_files:
        ...
        logger.info('%s', f"Created {notes_file}")
        ...
        logger.info('%s', f"Created {note_events_file}")
        ...
        logger.info('%s', f"Created {midi_file}")
    ...
    for i in range(60):
        ...
        for player in player_train:
            guitarset_ids_by_split["train"].append(
                guitarset_ids_by_split[f"player_{player}"][i]
            )
        guitarset_ids_by_split["validation"].append(
            guitarset_ids_by_split[f"player_{player_valid}"][i]
        )
        guitarset_ids_by_split["test"].append(
            guitarset_ids_by_split[f"player_{player_test}"][i]
        )
    ...
    for split in [
        ...
        for i, gid in enumerate(guitarset_ids_by_split[split]):
            ...
            wav_file = {}
            wav_file["hex"] = os.path.join(
                base_dir, "audio_hex-pickup_original", gid + "_" + "hex" + ".wav"
            )
            wav_file["hex_cln"] = os.path.join(
                base_dir, "audio_hex-pickup_debleeded", gid + "_" + "hex_cln" + ".wav"
            )
            wav_file["mic"] = os.path.join(
                base_dir, "audio_mono-mic", gid + "_" + "mic" + ".wav"
            )
            wav_file["mix"] = os.path.join(
                base_dir, "audio_mono-pickup_mix", gid + "_" + "mix" + ".wav"
            )
            for ver in wav_file:
        ...
        output_file = os.path.join(
            output_index_dir, f"{dataset_name}_{split}_file_list.json"
        )
        with open(output_file, "w") as f:
            json.dump(file_list, f, indent=4)
        logger.info('%s', f"Created {output_file}")
    ...
    file_list_all_path = os.path.join(
        output_index_dir, f"{dataset_name}_all_file_list.json"
    )
    with open(file_list_all_path, "r") as f:
        fl = json.load(f)
    file_list_all = {int(key): value for key, value in fl.items()}
    generate_pitch_shifted_wav_and_midi(
        file_list_all, min_pitch_shift=min_pitch_shift, max_pitch_shift=max_pitch_shift
    )

    # Create file_list.json for pitch shifted data
    for split in ["progression_1", "progression_2", "progression_3", "train", "all"]:
        src_file_list_path = os.path.join(
            output_index_dir, f"{dataset_name}_{split}_file_list.json"
        )
        ...
        for k, v in src_file_list.items():
            for pitch_shift in range(min_pitch_shift, max_pitch_shift):
                if pitch_shift == 0:
                    ...
                    shifted_audio_file = v["mix_audio_file"].replace(
                        ".wav", f"_pshift{pitch_shift}.wav"
                    )
                    assert os.path.isfile(shifted_audio_file) == True
                    file_list[k, pitch_shift]["mix_audio_file"] = shifted_audio_file
                    file_list[k, pitch_shift]["n_frames"] = get_audio_file_info(
                        shifted_audio_file
                    )[1]
                    file_list[k, pitch_shift]["pitch_shift"] = pitch_shift

                    shifted_midi_file = v["midi_file"].replace(
                        ".mid", f"_pshift{pitch_shift}.mid"
                    )
                    shifted_notes_file = v["notes_file"].replace(
                        "_notes", f"_pshift{pitch_shift}_notes"
                    )
                    shifted_note_events_file = v["note_events_file"].replace(
                        "_note_events", f"_pshift{pitch_shift}_note_events"
                    )
                    ...
                    file_list[k, pitch_shift][
                        "note_events_file"
                    ] = shifted_note_events_file
        assert len(file_list) == len(src_file_list) * (
            max_pitch_shift - min_pitch_shift
        )
        ...
        # Write json
        output_file = os.path.join(
            output_index_dir, f"{dataset_name}_{split}_pshift_file_list.json"
        )
        with open(output_file, "w") as f:
            json.dump(file_list, f, indent=4)
        logger.info('%s', f"Created {output_file}")
...
def create_filelist_by_style_guitarset16k(
    data_home: os.PathLike, dataset_name: str = "guitarset"
) -> None:
    ...
    file_list_pshift_all_path = os.path.join(
        output_index_dir, f"{dataset_name}_all_pshift_file_list.json"
    )
    ...
    for style in styles:
        ...
        for v in fl_pshift.values():
            if style in v["guitarset_id"]:
                ...
                i += 1
        output_file = os.path.join(
            output_index_dir, f"{dataset_name}_{style}_pshift_file_list.json"
        )
        with open(output_file, "w") as f:
        ...
        for v in fl.values():
            if style in v["guitarset_id"]:
                ...
                i += 1
        output_file = os.path.join(
            output_index_dir, f"{dataset_name}_{style}_file_list.json"
        )
        with open(output_file, "w") as f:
