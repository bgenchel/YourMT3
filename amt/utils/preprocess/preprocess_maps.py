...
from amt.utils.note2event import note2note_event, note_event2event
...
def create_note_event_and_note_from_midi(
    mid_file: str, id: str, ignore_pedal: bool = False
) -> Tuple[Dict, Dict]:
...
def preprocess_maps16k(
    data_home=os.PathLike, dataset_name="maps", ignore_pedal=False, sanity_check=False
) -> None:
    """
    Splits:
    ...
    for train_file in train_mid_files:
        if not any(
            songname in os.path.basename(train_file)
            for songname in songnames_in_test_files
        ):
    ...
    for i, mid_file in enumerate(all_mid_files):
        ...
        if sanity_check:
            # sanity check
            logger.info('%s', f"Sanity check for {i}: {maps_id}...")
        ...
        logger.info('%s', f"Overwrote {mid_file} with 120 bpm")
