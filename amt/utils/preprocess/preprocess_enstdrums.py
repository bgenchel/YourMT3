...
from amt.utils.audio import get_audio_file_info, load_audio_file, write_wav_file
...
from amt.config.vocabulary import ENST_DRUM_NOTES
...
def create_enst_audio_stem(drum_audio_file, accomp_audio_file, enst_id) -> Dict:
    ...
    drum_audio = (
        load_audio_file(drum_audio_file, dtype=np.int16) / 2**15
    )  # returns bytes
    audio_tracks.append(drum_audio.astype(np.float16))
    accomp_audio = (
        load_audio_file(accomp_audio_file, dtype=np.int16) / 2**15
    )  # returns bytes
...
def create_note_note_event_midi_from_enst_annotation(ann_file, enst_id):
    ...
    anns = [
        (float(line.split()[0]), re.sub("[^a-zA-Z]", "", line.split()[1]))
        for line in lines
    ]
...
def preprocess_enstdrums16k(data_home: os.PathLike, dataset_name="enstdrums") -> None:
    ...
    for i in [1, 2, 3]:
        ...
        for file in drummer_files:
            ...
            notes, note_events = create_note_note_event_midi_from_enst_annotation(
                ann_file, enst_id
            )
            ...
            if "minus-one" in file:  # unannotated accompaniment exists
                ...
                accomp_audio_file = os.path.join(
                    base_dir,
                    f"drummer_{i}",
                    "audio",
                    "accompaniment",
                ...
                assert os.path.exists(
                    accomp_audio_file
                ), f"{accomp_audio_file} does not exist"
                os.makedirs(
                    os.path.join(base_dir, f"drummer_{i}", "audio", "stem"),
                    exist_ok=True,
                )
                ...
                stem_content = create_enst_audio_stem(
                    drum_audio_file, accomp_audio_file, enst_id
                )
                # write audio stem
                np.save(stem_file, stem_content, allow_pickle=True, fix_imports=False)
                logger.info('%s', f"Created {stem_file}")
                ...
                accomp_mix_audio_r1 = (
                    stem_content["audio_array"][0]
                    / np.max(np.abs(stem_content["audio_array"][0]))
                    * 0.75
                    + stem_content["audio_array"][1]
                    / np.max(np.abs(stem_content["audio_array"][1]))
                    * 0.25
                )
                accomp_mix_audio_r1 = accomp_mix_audio_r1 / np.max(
                    np.abs(accomp_mix_audio_r1)
                )
                write_wav_file(accomp_mix_audio_file_r1, accomp_mix_audio_r1, 16000)
                logger.info('%s', f"Created {accomp_mix_audio_file_r1}")
                ...
                accomp_mix_audio_r2 = accomp_mix_audio_r2 / np.max(
                    np.abs(accomp_mix_audio_r2)
                )
                write_wav_file(accomp_mix_audio_file_r2, accomp_mix_audio_r2, 16000)
                logger.info('%s', f"Created {accomp_mix_audio_file_r2}")
            ...
            enst_info[enst_id] = {
                ...
                "midi_file": ann_file.replace(".txt", ".mid"),
                "program": (
                    stem_content["program"] if "minus-one" in file else notes["program"]
                ),
                "is_drum": (
                    stem_content["is_drum"] if "minus-one" in file else notes["is_drum"]
                ),
            }
    ...
    for split in [
        ...
        if split == "drummer_1_dtm":
            ...
            for enst_id in enst_ids:
                if enst_id.startswith("1_"):
                    ...
                    file_list[str(i)]["mix_audio_file"] = file_list[str(i)][
                        "mix_audio_file"
                    ].replace("accompaniment_mix_r2", "wet_mix")
            ...
            for enst_id in enst_ids:
                if enst_id.startswith("3_") and len(enst_info[enst_id]["program"]) == 2:
                    ...
                    file_list[str(i)]["mix_audio_file"] = file_list[str(i)][
                        "mix_audio_file"
                    ].replace("accompaniment_mix_r2", "accompaniment_mix_r1")
                    i += 1
            assert len(file_list) == 21
        ...
        # write json file
        output_index_file = os.path.join(
            output_index_dir, f"{dataset_name}_{split}_file_list.json"
        )
        with open(output_index_file, "w") as f:
            json.dump(file_list, f, indent=4)
        logger.info('%s', f"Created {output_index_file}")


def create_filelist_dtm_random_enstdrums16k(
    data_home: os.PathLike, dataset_name: str = "enstdrums"
) -> None:
    ...
    # Load all filelist
    file_list_all_dtm_path = os.path.join(
        output_index_dir, f"{dataset_name}_all_dtm_file_list.json"
    )
    file_list_all_dtp_path = os.path.join(
        output_index_dir, f"{dataset_name}_all_dtp_file_list.json"
    )
