...
def generate_corrected_midi(
    ...
    for track in org_mid.tracks[1:]:
        ...
        new_track = MidiTrack()
        new_track.append(MetaMessage("track_name", name=new_track_name, time=0))
        if program == DRUM_PROGRAM:
            new_track.append(Message("program_change", program=0, time=0, channel=9))
        else:
            new_track.append(
    ...
    logger.info('%s', f"Created {new_mid_file}")
...
def preprocess_rwc_pop_full16k(data_home="../../data", dataset_name="rwc_pop") -> None:
    ...
    with open(csv_file, "r") as f:
        ...
        for row in reader:
            id = row[2]
            mix_audio_file = os.path.join(
                base_dir, headers[0] + row[0], row[1] + " " + headers[1] + ".wav"
            )
    ...
    for id, info in rwc_all.items():
        ...
        generate_corrected_midi(
            org_mid_file, new_mid_file, filters=TRACK_NAME_FILTERS, prg2ch=PRG2CH
        )
    ...
    for id in UNUSED_IDS:
        rwc_all.pop(str(id))
    logger.info('%s', f"Number of used IDs: {len(rwc_all)}, Unused ids: {UNUSED_IDS}")
    ...
    output_index_file = os.path.join(
        output_index_dir, f"rwc_pop_{split}_file_list.json"
    )
