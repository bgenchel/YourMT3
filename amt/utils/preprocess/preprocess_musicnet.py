...
import numpy as np
from amt.utils.audio import get_audio_file_info
from amt.utils.midi import midi2note
...
# yapf: enable
MUSICNET_DISCARD_INFO = [
    "test_labels_midi/1759.mid",
    "test_labels_midi/1819.mid",
]  # duplicated midi files
...
def preprocess_musicnet16k(data_home=os.PathLike, dataset_name="musicnet") -> None:
    ...
    mid_em_pattern = os.path.join(
        base_dir, "*_em", "*.mid"
    )  # EM annotations for real performances (wav)
    ...
    def search_file_by_musicnet_id(musicnet_id, files):
        file_found = [
            f for f in files if musicnet_id in f
    ...
    for i in musicnet_ids:
        ...
        note_events_file = os.path.join(
            musicnet_dict[i]["label_file"][:-4] + "_note_events.npy"
        )
    ...
    for i in musicnet_ids:
        ...
        note_events_file = os.path.join(
            musicnet_dict[i]["mid_file"][:-4] + "_note_events.npy"
        )
        np.save(note_events_file, note_events, allow_pickle=True, fix_imports=False)
        logger.info('%s', f"Created {note_events_file}")

        # update musicnet_dict
        musicnet_dict[i]["duration_sec"] = max(
            notes["duration_sec"], musicnet_dict[i]["duration_sec"]
        )
        ...
        if i in musicnet_em_ids:
            ...
            notes_file = os.path.join(
                musicnet_dict[i]["mid_em_file"][:-4] + "_notes.npy"
            )
            np.save(notes_file, notes, allow_pickle=True, fix_imports=False)
            logger.info('%s', f"Created {notes_file}")
            ...
            note_events["is_drum"] = musicnet_dict[i]["is_drum"].copy()
            note_events_file = os.path.join(
                musicnet_dict[i]["mid_em_file"][:-4] + "_note_events.npy"
            )
            np.save(note_events_file, note_events, allow_pickle=True, fix_imports=False)
            logger.info('%s', f"Created {note_events_file}")
    ...
    split_dict["train_mt3"] = (
        whole_set
        - set(split_dict["validation_mt3"])
        - set(split_dict["test_mt3_acoustic"])
    )
    ...
    split_dict["train_mt3_em"] = (
        whole_set
        - set(split_dict["validation_mt3"])
        - set(split_dict["test_mt3_acoustic"])
        - MUSICNET_EM_MISSING_IDS
    )
    split_dict["train_thickstun_em"] = (
        whole_set - set(split_dict["test_thickstun_ext"]) - MUSICNET_EM_MISSING_IDS
    )
    ...
    for split in ("train_mt3_synth", "validation_mt3_synth"):
        ...
        for i, musicnet_id in enumerate(split_dict[split]):
            file_list[i] = {
                "musicnet_id": musicnet_id,
                "n_frames": get_audio_file_info(
                    musicnet_dict[musicnet_id]["wav_synth_file"]
                )[1],
                "mix_audio_file": musicnet_dict[musicnet_id]["wav_synth_file"],
                "notes_file": musicnet_dict[musicnet_id]["notes_file_synth"],
                "note_events_file": musicnet_dict[musicnet_id][
                    "note_events_file_synth"
                ],
        ...
        output_index_file = os.path.join(
            output_index_dir, f"musicnet_{split}_file_list.json"
        )
    ...
    for split in ("train_mt3_acoustic", "validation_mt3_acoustic", "test_mt3_acoustic"):
        ...
        for i, musicnet_id in enumerate(split_dict[split]):
            file_list[i] = {
                "musicnet_id": musicnet_id,
                "n_frames": get_audio_file_info(
                    musicnet_dict[musicnet_id]["wav_acoustic_file"]
                )[1],
    ...
    file_list_train_mt3_acoustic = json.load(
        open(
            os.path.join(output_index_dir, "musicnet_train_mt3_acoustic_file_list.json")
        )
    )
    ...
    for split in ("train_thickstun_em", "train_mt3_em"):
        ...
        synth_ids = (
            ...
            else split_dict["train_thickstun"]
        )
