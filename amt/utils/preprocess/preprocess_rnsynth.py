...
from amt.utils.audio import get_audio_file_info, load_audio_file, write_wav_file, guess_onset_offset_by_amp_envelope
from amt.utils.midi import note_event2midi
from amt.utils.note2event import note2note_event, sort_notes, validate_notes, trim_overlapping_notes, mix_notes
...
def create_sound_info(base_dir: os.PathLike, logger: logging.Logger,
    ...
    for i, (k, v) in enumerate(metadata.items()):
        if i % 5000 == 0:
            logger.info("Creating sound info %s / %s", i, len(metadata))
        nsynth_id = v['note']
