...
from amt.utils.audio import (
    get_audio_file_info,
    load_audio_file,
    write_wav_file,
    guess_onset_offset_by_amp_envelope,
)
...
from amt.utils.utils import assert_note_events_almost_equal
...
def bass_string_to_midi_pitch(
    string_number: int, fret: int, string_pitches=[28, 33, 38, 43, 48]
):
...
def create_note_event_and_note_from_midi(
    mid_file: str, id: str, program: int = 0, ignore_pedal: bool = True
) -> Tuple[Dict, Dict]:
...
def preprocess_idmt_smt_bass_16k(
    ...
    if edit_audio is True:
        for v in all_info_dict.values():
            ...
            logger.info('%s', f"Overwriting {audio_file} with silence prepended/appended")
    ...
    for id in all_info_dict.keys():
        ...
        _, _, _, _, pluck_style, _, string_num, fret_num = (
            os.path.basename(audio_file).split(".")[0].split("_")
        )
        ...
        if sanity_check:
            assert_note_events_almost_equal(
                note_events_dict["note_events"], note_events
            )
        ...
        np.save(
            note_events_file, note_events_dict, allow_pickle=True, fix_imports=False
        )
