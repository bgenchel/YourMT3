...
from amt.utils.note_event_dataclasses import NoteEvent, Note
from amt.utils.note2event import sort_notes, notes2pc_notes
from amt.utils.event2note import note_event2note
from sklearn.metrics import average_precision_score
from amt.utils.metrics_helper import (
    f1_measure,
    round_float,
    mir_eval_note_f1,
    mir_eval_frame_f1,
    mir_eval_melody_metric,
    extract_pitches_intervals_from_notes,
    extract_frame_time_freq_from_notes,
)
...
class AMTMetrics(Module):
    ...
    def bulk_update(
        self, metrics: Dict[str, Union[float, Dict[str, float], Tuple[float, ...]]]
    ) -> None:
    ...
    def bulk_update_errors(self, errors: Dict[str, Union[int, float]]) -> None:
        ...
        for error_type, count in errors.items():
            ...
            if isinstance(count, int) or isinstance(count, float):
                ...
                raise ValueError(
                    f"Count of error type '{error_type}' must be an integer or a float."
                )
    ...
    def bulk_compute(self) -> Dict[str, float]:
        ...
        for class_name in extra_classes + ["drum"]:
            ...
            _micro_onset_p_instr = computed_metrics.get(
                self._prefix + "micro_onset_p_" + class_name, None
            )
            _micro_onset_r_instr = computed_metrics.get(
                self._prefix + "micro_onset_r_" + class_name, None
            )
            if _micro_onset_p_instr is not None and _micro_onset_r_instr is not None:
                computed_metrics[self._prefix + "micro_onset_f_" + class_name] = (
                    f1_measure(_micro_onset_p_instr.item(), _micro_onset_r_instr.item())
                )
            # micro offset F1 for each instrument. 'drum' is usually not included.
            _micro_offset_p_instr = computed_metrics.get(
                self._prefix + "micro_offset_p_" + class_name, None
            )
            _micro_offset_r_instr = computed_metrics.get(
                self._prefix + "micro_offset_r_" + class_name, None
            )
            if _micro_offset_p_instr is not None and _micro_offset_r_instr is not None:
                computed_metrics[self._prefix + "micro_offset_f_" + class_name] = (
                    f1_measure(
        ...
        for class_name in extra_classes + ["drum"]:
            for onoff in ["micro_onset", "micro_offset"]:
                for pr in ["p", "r"]:
                    computed_metrics.pop(
                        self._prefix + onoff + "_" + pr + "_" + class_name, None
                    )
...
def compute_track_metrics(
    ...
    non_drum_metric = mir_eval_note_f1(
        pns_non_drum_pi["pitches"],
    ...
    if eval_vocab is None:
        ...
        for group_name, programs in eval_vocab.items():
            ...
            if add_pitch_class_metric is not None:
                if group_name.lower() in [g.lower() for g in add_pitch_class_metric]:
                    ...
                    pns_pc_group = extract_pitches_intervals_from_notes(
                        notes2pc_notes(pns_group)
                    )
                    rns_pc_group = extract_pitches_intervals_from_notes(
                        notes2pc_notes(rns_group)
                    )

                    _instr_pc_metric = mir_eval_note_f1(
                        pns_pc_group["pitches"],
            ...
            _instr_metric = mir_eval_note_f1(
                pns_group["pitches"],
                pns_group["intervals"],
                rns_group["pitches"],
                rns_group["intervals"],
            ...
            if add_melody_metric is not None:
                if group_name.lower() in [g.lower() for g in add_melody_metric]:
                    _melody_metric = mir_eval_melody_metric(
                        pns_group["pitches"],
                        pns_group["intervals"],
                        rns_group["pitches"],
                        rns_group["intervals"],
        ...
        if add_multi_f_metric is True:
            (
                drum_micro_onset_tp_sum,
                drum_micro_onset_tpfp_sum,
                drum_micro_onset_tpfn_sum,
            ) = (0.0, 0.0, 0.0)
            (
                non_drum_micro_offset_tp_sum,
                non_drum_micro_offset_tpfp_sum,
                non_drum_micro_offset_tpfn_sum,
            ) = (0.0, 0.0, 0.0)
