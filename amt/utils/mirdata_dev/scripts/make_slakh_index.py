...
def get_file_info(path):
    if os.path.exists(path):
        ...
        logger.info('%s', "warning: {} not found. check metadata for omitted files.".format(
            path))
        return [None, None]
...
def make_dataset_index(dataset_data_path, version):
    ...
    for split in splits:
        ...
        for mtrack_id in mtrack_ids:
            logger.info('%s', f'indexing multitrack: {mtrack_id}')
...
def main(args):
    ...
    logger.info('%s', f"A new index file is copied to {args.dataset_data_path}/mirdata_indexes/")
