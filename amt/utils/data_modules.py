...
from amt.utils.datasets_train import get_cache_data_loader
from amt.utils.datasets_eval import get_eval_dataloader
from amt.utils.datasets_helper import (
    create_merged_train_dataset_info,
    get_list_of_weighted_random_samplers,
)
...
from amt.config.data_presets import data_preset_single_cfg, data_preset_multi_cfg
...
class AMTDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        self.num_train_samplers = (
            shared_cfg["BSZ"]["train_local"] // shared_cfg["BSZ"]["train_sub"]
        )
    ...
    def set_merged_train_data_info(self) -> None:
        ...
        logger.info('%s', f"AMTDataModule: Added {len(self.train_data_info['merged_file_list'])} files from {self.train_data_info['n_datasets']} datasets to the training set.")
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            ...
            actual_train_num_samples_per_epoch = (
                self.train_num_samples_per_epoch // shared_cfg["BSZ"]["train_local"]
                if self.train_num_samples_per_epoch
                else None
            )
            samplers = get_list_of_weighted_random_samplers(
                num_samplers=self.num_train_samplers,
    ...
    def train_dataloader(self) -> Any:
        ...
        for i, args_dict in enumerate(self.train_data_args):
            loaders[f"data_loader_{i}"] = get_cache_data_loader(
    ...
    def val_dataloader(self) -> Any:
        ...
        for args_dict in self.val_data_args:
            ...
            loaders[dataset_name] = get_eval_dataloader(
                **args_dict, dataloader_config=shared_cfg["DATAIO"]
            )
        return loaders
