...
from amt.utils.audio import get_segments_from_numpy_array, load_audio_file
from amt.utils.augment import (
...
from amt.utils.note2event import (
    ...
    slice_note_events_and_ties,
)
from amt.utils.note2event import pitch_shift_note_events
from amt.utils.note_event_dataclasses import NoteEventListsBundle
...
class CachedAudioDataset(Dataset):
    ...
    def __init__(
        ...
        self.num_decoding_channels = (
            task_manager.num_decoding_channels
        ...
        if stem_xaug_policy is not None:
            ...
            self.precomputed_prob_stop_at_k = combined_survival_and_stop(
                max_k=stem_xaug_policy["max_k"],
                tau=stem_xaug_policy["tau"],
                alpha=stem_xaug_policy["alpha"],
            )[1]
        ...
        if num_files_cache is None:
            if stem_xaug_policy is None:
                ...
                self.num_files_cache = (
                    max(4, stem_xaug_policy["max_k"] + 1) * sub_batch_size
                )
            ...
            raise ValueError(
                f"📙 num_files_cache must be an integer or None. Got {num_files_cache}."
            )

        self.seg_read_size = 1  # np.ceil(sub_batch_size / num_files_cache).astype(int)
        self.num_cached_seg_per_file = sub_batch_size
        logger.info('%s', f"📘 caching {self.num_cached_seg_per_file} segments per file.")
    ...
    def __getitem__(
        ...
        sampled_data, sampled_ids = self._get_rand_segments_from_cache(
            num_segments=self.sub_batch_size
        )  # sampled_data is deepcopy of sampled cached instances
    ...
    def _post_mix_augment(self, sampled_data: Dict[str, Any]) -> None:
        ...
        if self.pitch_shift_range is None:
            ...
            sampled_data["pitch_shift_steps"] = np.random.randint(
                self.pitch_shift_range[0], self.pitch_shift_range[1] + 1
            ) * np.ones(self.sub_batch_size)
            ...
            note_event_segments = sampled_data["note_event_segments"]
            for i, (note_events, tie_note_events, start_time) in enumerate(
                list(zip(*note_event_segments.values()))
            ):
                note_events = pitch_shift_note_events(
                    note_events, sampled_data["pitch_shift_steps"][i], use_deepcopy=True
                )
                tie_note_events = pitch_shift_note_events(
                    tie_note_events,
                    sampled_data["pitch_shift_steps"][i],
                    use_deepcopy=True,
                )

    def _augment_stems_and_audio_submix_in_place(
        self, sampled_data: Dict[str, Any], sampled_ids: np.ndarray
    ) -> None:
        """Augment stems and submix audio"""

        if self.stem_iaug_prob is None or self.stem_iaug_prob == 1.0:
            # no augmentation at all
            audio_random_submix_processor(
                sampled_data=sampled_data, random_amp_range=self.random_amp_range
            )
            return
            ...
            intra_stem_augment_processor(
                ...
                prob=self.stem_iaug_prob,
                submix_audio=False,
            )  # submix_audio=False to postpone audio mixing until cross-stem augmentation
            cross_stem_augment_processor(
    ...
    def _get_rand_segments_from_cache(
        ...
        sample_excluding_ids: Optional[np.ndarray] = None,
    ) -> Tuple[NoteEventListsBundle, np.ndarray]:
        ...
        if num_segments == "max":
            ...
            raise ValueError(
                f"num_segments must be int or 'max', but got {num_segments}"
            )
        ...
        if sample_excluding_ids is None:
            ...
            sampled_ids = np.random.permutation(
                list(set(np.arange(self.num_files_cache)) - set(sample_excluding_ids))
            )[:n]
        ...
        for d in selected_files:
            if use_ordered_read_pos is False:
                start = np.random.randint(
                    0, self.num_cached_seg_per_file - self.seg_read_size + 1
                )
                end = start + self.seg_read_size
        ...
        return (
            sampled_data,
            sampled_ids,
        )  # Note that the data returned is mutable instance.
    ...
    def _update_cache(self, index) -> None:
        ...
        if (
            "stem_file" in self.file_list[index].keys()
            and self.file_list[index]["stem_file"] != None
        ):
            audio_data = np.load(
                self.file_list[index]["stem_file"], allow_pickle=True
            ).tolist()  # dict with 'audio_array' having shape (n_stems, n_frames)
            data["has_stems"] = True
        elif "mix_audio_file" in self.file_list[index].keys():
            wav_data = load_audio_file(
                self.file_list[index]["mix_audio_file"], fs=self.fs, dtype=np.float32
            )
            audio_data = {
            ...
            raise ValueError(
                f"📕 No stem_file or mix_audio_file found in the file list."
            )
        ...
        audio_segments = get_segments_from_numpy_array(
            audio_data["audio_array"],
            self.seg_len_frame,
            start_frame_indices=start_frame_indices,
            dtype=np.float32,
        )  # audio_segments: (n_segs, n_stems, n_frames)
    ...
    def _init_cache(self, index_for_init_cache: Optional[List[int]] = None):
        with Timer() as t:
            ...
            logger.info('%s', f"💿 Initializing cache with max_size={self.cache.max_size}")
            if index_for_init_cache is not None:
                ...
                rand_ids = np.random.choice(
                    np.arange(len(self)), size=self.num_files_cache, replace=False
                )
                for i in rand_ids:
                    self._update_cache(i)

            # Initialize an infinite cache read position generator
            self._cache_read_pos_generator = cycle(
                np.arange(0, self.num_cached_seg_per_file, self.seg_read_size)
            )
...
def collate_fn(
    batch: Tuple[torch.FloatTensor, torch.LongTensor], local_batch_size: int
...
def get_cache_data_loader(
    ...
    if dataset_name is None and split is None and file_list is None:
        ...
        (dataset_name is not None and split is not None and file_list is None)
        ...
        dataset_name is None and split is None and file_list is not None
    ) and isinstance(file_list, dict):
        pass
    else:
