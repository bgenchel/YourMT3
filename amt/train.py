...
def main():
    ...
    model = YourMT3(
        ...
        optimizer_name=(
            "CPUAdam" if "offload" in args.strategy.lower() else args.optimizer
        ),
        scheduler_name=args.scheduler.lower(),
        base_lr=(
            float(args.base_learning_rate) if args.base_learning_rate != None else None
        ),
    ...
    if trainer.global_rank == 0:
        wandb_logger.experiment.config.update(
            {"audio_cfg": model.audio_cfg}, allow_val_change=True
        )
        wandb_logger.experiment.config.update(
            {"model_cfg": model.model_cfg}, allow_val_change=True
        )
