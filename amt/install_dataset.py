...
from amt.utils.preprocess.generate_dataset_stats import (
    generate_dataset_stats_for_all_datasets,
    update_dataset_stats_for_new_dataset,
)
from amt.utils.mirdata_dev.datasets import slakh16k
from amt.utils.preprocess.preprocess_slakh import (
    preprocess_slakh16k,
    add_program_and_is_drum_info_to_file_list,
)
from amt.utils.preprocess.preprocess_musicnet import preprocess_musicnet16k
from amt.utils.preprocess.preprocess_maps import preprocess_maps16k
from amt.utils.preprocess.preprocess_maestro import preprocess_maestro16k
from amt.utils.preprocess.preprocess_guitarset import (
    preprocess_guitarset16k,
    create_filelist_by_style_guitarset16k,
)
from amt.utils.preprocess.preprocess_enstdrums import (
    preprocess_enstdrums16k,
    create_filelist_dtm_random_enstdrums16k,
)
from amt.utils.preprocess.preprocess_mir_st500 import preprocess_mir_st500_16k
from amt.utils.preprocess.preprocess_cmedia import preprocess_cmedia_16k
from amt.utils.preprocess.preprocess_rwc_pop_full import preprocess_rwc_pop_full16k
from amt.utils.preprocess.preprocess_rwc_pop import preprocess_rwc_pop16k
from amt.utils.preprocess.preprocess_egmd import preprocess_egmd16k
from amt.utils.preprocess.preprocess_mir1k import preprocess_mir1k_16k
from amt.utils.preprocess.preprocess_urmp import preprocess_urmp16k
from amt.utils.preprocess.preprocess_idmt_smt_bass import preprocess_idmt_smt_bass_16k
from amt.utils.preprocess.preprocess_geerdes import preprocess_geerdes16k
from amt.utils.utils import (
    download_and_extract,
)  # , download_and_extract_zenodo_restricted
...
def install_maps(data_home=os.PathLike, no_down=False, sanity_check=False) -> None:
    if not no_down:
        url = (
            "https://zenodo.org/record/7812075/files/maps_yourmt3_16k.tar.gz?download=1"
        )
        checksum = "6b070d162c931cd5e69c16ef2398a649"
        download_and_extract(data_home, url, remove_tar_file=True, check_sum=checksum)
    preprocess_maps16k(
        data_home, dataset_name="maps", ignore_pedal=False, sanity_check=sanity_check
    )
...
def install_maestro(data_home=os.PathLike, no_down=False, sanity_check=False) -> None:
    if not no_down:
        ...
        download_and_extract(data_home, url, remove_tar_file=True, check_sum=checksum)
    preprocess_maestro16k(
        data_home, dataset_name="maestro", ignore_pedal=False, sanity_check=sanity_check
    )
...
def install_egmd(data_home, no_down=False) -> None:
    if not no_down:
        url = (
            "https://zenodo.org/record/7831072/files/egmc_yourmt3_16k.tar.gz?download=1"
        )
...
def install_mirst500(
    data_home, zenodo_token, no_down=False, sanity_check=True, apply_correction=False
) -> None:
    ...
    if not no_down:
        ...
        download_and_extract(
            data_home, url, check_sum=checksum, zenodo_token=zenodo_token
        )
    preprocess_mir_st500_16k(
        data_home, dataset_name="mir_st500", sanity_check=sanity_check
    )
...
def install_urmp(data_home, no_down=False) -> None:
    if not no_down:
        url = (
            "https://zenodo.org/record/8021437/files/urmp_yourmt3_16k.tar.gz?download=1"
        )
...
def install_idmt_smt_bass(data_home, no_down=False) -> None:
    if not no_down:
        ...
        download_and_extract(data_home, url, remove_tar_file=True, check_sum=checksum)
    preprocess_idmt_smt_bass_16k(
        data_home, dataset_name="idmt_smt_bass", sanity_check=True, edit_audio=False
    )  # the donwloaded audio has already been edited
...
def install_geerdes(data_home) -> None:
    try:
        ...
        logger.info('%s', e)
        logger.info('%s', "Geerdes dataset is not available for download. Please contact the dataset provider.")
...
def get_cached_zenodo_token() -> str:
    ...
    if not os.path.exists(".cached_zenodo_token"):
        raise Exception(
            "Cached Zenodo token not found. Please enter your Zenodo token."
        )
    # read cached token
    with open(".cached_zenodo_token", "r") as f:
        zenodo_token = f.read().strip()
        logger.info('%s', f"Using cached Zenodo token: {zenodo_token}")
    return zenodo_token
...
def cache_zenodo_token(zenodo_token: str) -> None:
    ...
    logger.info('%s', "Your Zenodo token is cached.")


def option_prompt(data_home: os.PathLike, no_download: bool = False) -> None:
    logger.info('%s', "Select the dataset(s) to install (enter comma-separated numbers):")
    logger.info('%s', "1. Slakh")
    logger.info('%s', "2. MusicNet")
    logger.info('%s', "3. MAPS")
    logger.info('%s', "4. Maestro")
    logger.info('%s', "5. GuitarSet")
    logger.info('%s', "6. ENST-drums")
    logger.info('%s', "7. EGMD")
    logger.info('%s', "8. MIR-ST500 ** Restricted Access **")
    logger.info('%s', "9. CMedia ** Restricted Access **")
    logger.info('%s', "10. RWC-Pop (Bass and Full) ** Restricted Access **")
    logger.info('%s', "11. MIR-1K (NOT SUPPORTED)")
    logger.info('%s', "12. URMP")
    logger.info('%s', "13. IDMT-SMT-Bass")
    logger.info('%s', "14. Random-NSynth")
    logger.info('%s', "15. Geerdes")
    logger.info('%s', "16. Regenerate Dataset Stats (experimental)")
    logger.info('%s', "17. Request Token for ** Restricted Access **")
    logger.info('%s', "18. Exit")
    ...
    if "18" in choices:
        ...
        for c in choices:
            if int(c) in [8, 9, 10]:
                if no_download is True:
                    ...
                    zenodo_token = input(
                        "Enter Zenodo token, or press enter to use the cached token:"
                    )
                    if zenodo_token == "":
        ...
        if "17" in choices:
            ...
            logger.info('%s', "Upon submitting your request, you will receive an email with a link labeled 'Access the record'.")
            logger.info('%s', "Copy the token that follows 'token=' in that link.")
        if not any(int(c) in range(16) for c in choices):
            logger.info('%s', "Invalid choice(s). Please enter valid numbers separated by commas.")
...
if __name__ == "__main__":
    ...
    if args.data_home is None:
        from amt.config.config import shared_cfg
