...
from amt.model.conformer_helper import ConformerYMT3Config, ConformerYMT3PreTrainedModel
from amt.model.positional_encoding import (
...
class ConformerYMT3SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.position_encoding_type == "relative":
            if relative_position_embeddings is None:
                raise ValueError(
                    "`relative_position_embeddings` has to be defined when `self.position_encoding_type =="
                    " 'relative'"
                )
...
class ConformerYMT3Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [ConformerYMT3EncoderLayer(config) for _ in range(config.num_layers)]
        )
