...
from amt.config.config import DEEPSPEED_CFG

# PyTorch Lightning logs Trainer setup messages (device availability, etc.)
# through this package namespace.
logging.getLogger("pytorch_lightning").setLevel(logging.ERROR)
logger = logging.getLogger(__name__)


def initialize_trainer(
    args: argparse.Namespace, stage: Literal["train", "test"] = "train"
) -> Tuple[pl.Trainer, WandbLogger, dict]:
    ...
    lightning_dir = os.path.join(
        shared_cfg["WANDB"]["save_dir"], args.project, args.exp_id
    )
    ...
    if os.path.exists(os.path.join(last_ckpt_path)):
        logger.info("%s", f"Resuming from {last_ckpt_path}")
    elif stage == "train":
        logger.info(
            "%s", f"No checkpoint found in {last_ckpt_path}. Starting from scratch"
        )
        last_ckpt_path = None
    else:
...
def update_config(args, shared_cfg, stage: Literal["train", "test"] = "train"):
    ...
    if stage == "train":
        ...
        if args.stem_iaug_prob is not None:
            shared_cfg["AUGMENTATION"]["train_stem_iaug_prob"] = float(
                args.stem_iaug_prob
            )

        if args.xaug_max_k is not None:
            shared_cfg["AUGMENTATION"]["train_stem_xaug_policy"]["max_k"] = int(
                args.xaug_max_k
            )
        if args.xaug_tau is not None:
            shared_cfg["AUGMENTATION"]["train_stem_xaug_policy"]["tau"] = float(
                args.xaug_tau
            )
        if args.xaug_alpha is not None:
            shared_cfg["AUGMENTATION"]["train_stem_xaug_policy"]["alpha"] = float(
                args.xaug_alpha
            )
        if args.xaug_no_instr_overlap is not None:
            shared_cfg["AUGMENTATION"]["train_stem_xaug_policy"]["no_instr_overlap"] = (
                bool(args.xaug_no_instr_overlap)
            )
        if args.xaug_no_drum_overlap is not None:
            shared_cfg["AUGMENTATION"]["train_stem_xaug_policy"]["no_drum_overlap"] = (
                bool(args.xaug_no_drum_overlap)
            )
        if args.uhat_intra_stem_augment is not None:
            shared_cfg["AUGMENTATION"]["train_stem_xaug_policy"][
                "uhat_intra_stem_augment"
            ] = bool(args.uhat_intra_stem_augment)
        ...
        logger.info(
            "%s",
            f"Random amp range: {random_amp_range}\n"
    ...
    if shared_cfg["TOKENIZER"]["max_shift_steps"] == "auto":
        ...
        max_shift_steps = (input_frames / fs) // (
            shift_steps_ms / 1000
        ) + 2  # 206 by default
    ...
    assert isinstance(args.task_cond_decoder, bool) and isinstance(
        args.task_cond_encoder, bool
    )
    ...
    if args.encoder_position_encoding_type != "default":
        if args.encoder_position_encoding_type in ["None", "none", "0"]:
            model_cfg["encoder"][model_cfg["encoder_type"]][
            ...
            model_cfg["encoder"][model_cfg["encoder_type"]][
                "position_encoding_type"
            ] = str(args.encoder_position_encoding_type)
        else:
            raise ValueError(
                f"Encoder PE type {args.encoder_position_encoding_type} not supported"
            )
    if args.decoder_position_encoding_type != "default":
        if args.decoder_position_encoding_type in ["None", "none", "0"]:
            ...
            model_cfg["decoder"][model_cfg["decoder_type"]][
                "position_encoding_type"
            ] = str(args.decoder_position_encoding_type)
        else:
            raise ValueError(
                f"Decoder PE {args.decoder_position_encoding_type} not supported"
            )
    ...
    if args.num_perceiver_tf_blocks != None:
        model_cfg["encoder"]["perceiver-tf"]["num_blocks"] = int(
            args.num_perceiver_tf_blocks
        )
    ...
    if args.num_perceiver_tf_temporal_transformers_per_block != None:
        model_cfg["encoder"]["perceiver-tf"]["num_temporal_transformers_per_block"] = (
            int(args.num_perceiver_tf_temporal_transformers_per_block)
        )
    if args.attention_to_channel != None:
        model_cfg["encoder"]["perceiver-tf"]["attention_to_channel"] = bool(
            args.attention_to_channel
        )
    if args.sca_use_query_residual != None:
        model_cfg["encoder"]["perceiver-tf"]["sca_use_query_residual"] = bool(
            args.sca_use_query_residual
        )
    ...
    if args.ff_widening_factor != None:
        model_cfg["encoder"]["perceiver-tf"]["ff_widening_factor"] = int(
            args.ff_widening_factor
        )
    if args.moe_num_experts != None:
        model_cfg["encoder"]["perceiver-tf"]["moe_num_experts"] = int(
    ...
    if args.rotary_type != None:
        assert (
            len(args.rotary_type) == 3
        ), "rotary_type must be a 3-letter string (e.g. 'ppl': 'pixel' for SCA, 'pixel' for latent, 'lang' for temporal transformer)"
        model_cfg["encoder"]["perceiver-tf"]["rotary_type_sca"] = str(args.rotary_type)[
            0
        ]
        model_cfg["encoder"]["perceiver-tf"]["rotary_type_latent"] = str(
            args.rotary_type
        )[1]
        model_cfg["encoder"]["perceiver-tf"]["rotary_type_temporal"] = str(
            args.rotary_type
        )[2]
    if args.rope_apply_to_keys != None:
        model_cfg["encoder"]["perceiver-tf"]["rope_apply_to_keys"] = bool(
            args.rope_apply_to_keys
        )
    if args.rope_partial_pe != None:
        model_cfg["encoder"]["perceiver-tf"]["rope_partial_pe"] = bool(
            args.rope_partial_pe
        )

    if args.decoder_ff_layer_type != None:
        model_cfg["decoder"][model_cfg["decoder_type"]]["ff_layer_type"] = str(
            args.decoder_ff_layer_type
        )
    if args.decoder_ff_widening_factor != None:
        model_cfg["decoder"][model_cfg["decoder_type"]]["ff_widening_factor"] = int(
            args.decoder_ff_widening_factor
        )
    ...
    if stage == "train":
        if args.encoder_dropout_rate != None:
            model_cfg["encoder"][model_cfg["encoder_type"]]["dropout_rate"] = float(
                args.encoder_dropout_rate
            )
        if args.decoder_dropout_rate != None:
            model_cfg["decoder"][model_cfg["decoder_type"]]["dropout_rate"] = float(
                args.decoder_dropout_rate
            )
