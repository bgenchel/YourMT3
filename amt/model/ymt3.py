...
from amt.model.t5mod import T5EncoderYMT3, T5DecoderYMT3, MultiChannelT5Decoder
from amt.model.t5mod_helper import task_cond_dec_generate
from amt.model.perceiver_mod import PerceiverTFEncoder
from amt.model.perceiver_helper import PerceiverTFConfig
from amt.model.conformer_mod import ConformerYMT3Encoder
from amt.model.conformer_helper import ConformerYMT3Config
from amt.model.lm_head import LMHead
from amt.model.pitchshift_layer import PitchShiftLayer
from amt.model.spectrogram import get_spectrogram_layer_from_audio_cfg
from amt.model.conv_block import PreEncoderBlockRes3B
from amt.model.conv_block import (
    PreEncoderBlockHFTT,
    PreEncoderBlockRes3BHFTT,
)  # added for hFTT-like pre-encoder
from amt.model.projection_layer import (
    get_projection_layer,
    get_multi_channel_projection_layer,
)
from amt.model.optimizers import get_optimizer
from amt.model.lr_scheduler import get_lr_scheduler

from amt.utils.note_event_dataclasses import Note
from amt.utils.note2event import mix_notes
from amt.utils.event2note import (
    merge_zipped_note_events_and_ties_to_notes,
    DECODING_ERR_TYPES,
)
from amt.utils.metrics import compute_track_metrics
from amt.utils.metrics import AMTMetrics

# from utils.utils import write_model_output_as_npy
from amt.utils.utils import (
    write_model_output_as_midi,
    create_inverse_vocab,
    write_err_cnt_as_json,
)
from amt.utils.utils import Timer
from amt.utils.task_manager import TaskManager

from amt.config.config import audio_cfg as default_audio_cfg
from amt.config.config import model_cfg as default_model_cfg
from amt.config.config import shared_cfg as default_shared_cfg
from amt.config.config import T5_BASE_CFG
...
class YourMT3(pl.LightningModule):
        ...
        **kwargs: Any,
        ...
        if pretrained is True:
            raise NotImplementedError(
                "Pretrained model is not supported in this version."
            )
        ...
        if model_cfg is None:
            model_cfg = (
                default_model_cfg  # default config, not overwritten by args of trainer
            )
        if audio_cfg is None:
            audio_cfg = (
                default_audio_cfg  # default config, not overwritten by args of trainer
            )
        if shared_cfg is None:
            shared_cfg = (
                default_shared_cfg  # default config, not overwritten by args of trainer
            )
        ...
        self.spectrogram, spec_output_shape = get_spectrogram_layer_from_audio_cfg(
            audio_cfg
        )  # can be spec or melspec; output_shape is (T, F)
        ...
        self.encoder_type = model_cfg[
            "encoder_type"
        ]  # {"t5", "perceiver-tf", "conformer"}
        ...
        # Positional Encoding
        if (
            isinstance(model_cfg["num_max_positions"], str)
            and model_cfg["num_max_positions"] == "auto"
        ):
            encoder_config["num_max_positions"] = int(
                model_cfg["feat_length"] + self.task_manager.max_task_token_length + 10
            )
        ...
        if model_cfg["pre_encoder_type"] == "default":
            model_cfg["pre_encoder_type"] = model_cfg["pre_encoder_type_default"].get(
                model_cfg["encoder_type"], None
            )
        elif model_cfg["pre_encoder_type"] in [None, "none", "None", "0"]:
            model_cfg["pre_encoder_type"] = None
        if model_cfg["pre_decoder_type"] == "default":
            model_cfg["pre_decoder_type"] = (
                model_cfg["pre_decoder_type_default"]
                .get(model_cfg["encoder_type"])
                .get(model_cfg["decoder_type"], None)
            )
        ...
        if self.pre_encoder_type in ["conv", "conv1d_t", "conv1d_f"]:
            ...
            pre_enc_output_shape = (
                spec_output_shape[0],
                spec_output_shape[1] // 2**3,
                model_cfg["conv_out_channels"],
        ...
        if isinstance(model_cfg["d_feat"], str) and model_cfg["d_feat"] == "auto":
            if (
                self.encoder_type == "perceiver-tf"
                and encoder_config["attention_to_channel"] is True
            ):
                model_cfg["d_feat"] = pre_enc_output_shape[
                    -2
                ]  # TODO: better readablity
            else:
                model_cfg["d_feat"] = pre_enc_output_shape[
                    -1
                ]  # C of (T, F, C) or F or (T, F)

        if self.encoder_type == "perceiver-tf" and isinstance(
            encoder_config["d_model"], str
        ):
        ...
        if (
            isinstance(model_cfg["vocab_size"], str)
            and model_cfg["vocab_size"] == "auto"
        ):
        ...
        if (
            ...
            model_cfg["num_max_positions"] = int(
                max(model_cfg["feat_length"], model_cfg["event_length"])
        ...
        if self.encoder_type == "perceiver-tf" and self.decoder_type == "t5":
            ...
            encoder_output_shape = (
                t,
                encoder_config["num_latents"],
                encoder_config["d_latent"],
            )  # (T, K, D_source)
            ...
            proj_layer = get_multi_channel_projection_layer(
                input_shape=encoder_output_shape,
                output_shape=decoder_input_shape,
                proj_type=self.pre_decoder_type,
            )
        ...
        if self.encoder_type in ["t5", "conformer"]:
            encoder_config["num_max_positions"] = decoder_config[
                "num_max_positions"
            ] = model_cfg["num_max_positions"]
        else:  # perceiver-tf uses separate positional encoding
            encoder_config["num_max_positions"] = model_cfg["feat_length"]
            decoder_config["num_max_positions"] = model_cfg["num_max_positions"]
        encoder_config["vocab_size"] = decoder_config["vocab_size"] = model_cfg[
            "vocab_size"
        ]
        ...
        if write_output_dir is not None:
            if write_output_vocab is None:
                from amt.config.vocabulary import program_vocab_presets

                self.midi_output_vocab = program_vocab_presets["gm_ext_plus"]
            else:
                self.midi_output_vocab = write_output_vocab
            self.midi_output_inverse_vocab = create_inverse_vocab(
                self.midi_output_vocab
            )
    ...
    def set_encoder_decoder(self) -> None:
        ...
        if self.decoder_type == "t5":
            ...
            self.decoder = MultiChannelT5Decoder(
                self.model_cfg["decoder"]["multi-t5"], t5_config
            )
    ...
    def setup(self, stage: str) -> None:
        ...
        if self.hparams.eval_vocab is None:
            ...
            extra_classes_per_dataset = [
                list(v.keys()) if v is not None else None
                for v in self.hparams.eval_vocab
        ...
        if self.hparams.add_melody_metric_to_singing is True:
            extra_metrics = [
                "melody_rpa_Singing Voice",
                "melody_rca_Singing Voice",
                "melody_oa_Singing Voice",
            ]
        ...
        )  # e.g. ['Strings', 'Winds', 'Guitar', 'Piano']
        dm = self.trainer.datamodule

        # Train/Vaidation-only
        if stage == "fit":
            self.val_metrics_macro = AMTMetrics(
                prefix=f"validation/macro_", extra_classes=extra_classes_unique
            )
            self.val_metrics = (
                nn.ModuleList()
            )  # val_metric is a list of AMTMetrics objects
            for i in range(dm.num_val_dataloaders):
                self.val_metrics.append(
                    AMTMetrics(
                        prefix=f"validation/({dm.get_val_dataset_name(i)})",
            ...
            if self.shared_cfg["AUGMENTATION"]["train_pitch_shift_range"] in [
                None,
                [0, 0],
            ]:
                ...
                input_shape = (
                    self.shared_cfg["BSZ"]["train_local"],
                    1,
                    self.audio_cfg["input_frames"],
                )
                self.pitchshift = PitchShiftLayer(
                    pshift_range=self.shared_cfg["AUGMENTATION"][
                        "train_pitch_shift_range"
                    ],
        ...
        # Test-only
            ...
            for i in range(dm.num_test_dataloaders):
                self.test_metrics.append(
                    AMTMetrics(
                        prefix=f"test/({dm.get_test_dataset_name(i)})",
                        extra_classes=extra_classes_per_dataset[i],
            ...
            if self.test_pitch_shift_layer is not None:
                ...
                self.pitchshift = PitchShiftLayer(
                    pshift_range=[
                        self.test_pitch_shift_semitone,
                        self.test_pitch_shift_semitone,
                    ]
                )
    ...
    def configure_optimizers(self) -> None:
        ...
        optimizer, base_lr = get_optimizer(
            models_dict=self.named_parameters(),
        ...
        if (
            self.hparams.optimizer_name.lower() == "adafactor"
            and self.hparams.base_lr == None
        ):
            logger.info('%s', "Using AdaFactor with auto learning rate and no scheduler")
            return [optimizer]
        if self.hparams.optimizer_name.lower() == "dadaptadam":
            logger.info('%s', "Using dAdaptAdam with auto learning rate and no scheduler")
            return [optimizer]
        elif self.hparams.base_lr == None:
            logger.info('%s', f"Using default learning rate {base_lr} of {self.hparams.optimizer_name} as base learning rate.")
    ...
    def forward(
        ...
        **kwargs,
        ...
        dec_inputs_embeds = self.embed_tokens(
            dec_input_ids
        )  # t5:(B, N, D), multi-t5:(B, C, N, D)
        ...
        if self.model_cfg["tie_word_embeddings"] is True:
            dec_hs = dec_hs * (
                self.model_cfg["decoder"][self.decoder_type]["d_model"] ** -0.5
            )
    ...
    def inference(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            task_embed = self.embed_tokens(task_tokens)  # (B, task_len, 512)
            x = torch.cat([task_embed, x], dim=1)  # (B, task_len + 256, 512)
        enc_hs = self.encoder(inputs_embeds=x)[
            "last_hidden_state"
        ]  # (B, task_len + 256, 512)
        ...
        pred_ids = task_cond_dec_generate(
            decoder=self.decoder,
        ...
        )  # (B, task_len + N) or (B, C, task_len + N)
        if pred_ids.dim() == 2:
    ...
    def inference_file(
        ...
        for i in range(0, n_items, bsz):
            if i + bsz > n_items:  # last batch can be smaller
                ...
                if note_token_array is not None:
                    target_tokens = note_token_array[i : i + bsz].to(
                        self.device
                    )  # (bsz, token_len)
                if task_token_array is not None and task_token_array.numel() > 0:
    ...
    def training_step(self, batch, batch_idx) -> torch.Tensor:
        ...
        # 'dataset3': ...
        # }
        audio_segments, note_tokens, pshift_steps = [
            torch.cat(t, dim=0) for t in zip(*batch.values())
        ]
        ...
        if self.pitchshift is not None:
            ...
            audio_segments = torch.cat(
                [
                    self.pitchshift(a, p[0].item())
                    for a, p in zip(audio_segments, pshift_steps)
                ],
                dim=0,
            )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0) -> Dict:
        ...
        if self.task_manager.num_decoding_channels == 1:
            ...
            bsz = (
                self.shared_cfg["BSZ"]["validation"]
                // self.task_manager.num_decoding_channels
                * 3
            )
        ...
        with Timer() as t:
            pred_token_array_file, loss = self.inference_file(
                bsz, audio_segments, note_token_array, task_token_array
            )
            """
            ...
            for ch in range(num_channels):
                pred_token_array_ch = [
                    arr[:, ch, :] for arr in pred_token_array_file
                ]  # (B, L)
                zipped_note_events_and_tie, list_events, ne_err_cnt = (
                    self.task_manager.detokenize_list_batches(
            ...
            pred_notes = mix_notes(
                pred_notes_in_file
            )  # This is the mixed notes from all channels

            if self.hparams.write_output_dir is not None:
                track_info = [
                    notes_dict[k] for k in notes_dict.keys() if k.endswith("_id")
                ][0]
                dataset_info = [k for k in notes_dict.keys() if k.endswith("_id")][0][
                    :-3
                ]
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0) -> Dict:
        ...
        # Test pitch shift layer: debug only
        if (
            self.test_pitch_shift_layer is not None
            and self.test_pitch_shift_semitone != 0
        ):
        ...
        if (
            self.test_pitch_shift_layer is not None
            and self.hparams.write_output_dir is not None
        ):
            pred_token_array_file, loss, x_ps = self.inference_file(
                bsz, audio_segments, None, None
            )
        else:
            pred_token_array_file, loss = self.inference_file(
                bsz, audio_segments, None, None
        ...
        if len(pred_token_array_file) > 0:
            ...
            if (
                ...
                wav_output_dir = os.path.join(
                    self.hparams.write_output_dir, f"model_output_{dataset_info}"
                )
                os.makedirs(wav_output_dir, exist_ok=True)
                wav_output_file = os.path.join(
                    wav_output_dir,
                    f"{track_info}_ps_{self.test_pitch_shift_semitone}.wav",
                )
                torchaudio.save(
                    wav_output_file, x_ps.squeeze(1), 16000, bits_per_sample=16
                )
            ...
            drum_metric, non_drum_metric, instr_metric = compute_track_metrics(
                ...
                add_melody_metric=(
                    ["Singing Voice"]
                    if self.hparams.add_melody_metric_to_singing
                    else None
                ),
            ...
            if self.hparams.test_optimal_octave_shift:
                ...
                drum_metric_plus, non_drum_metric_plus, instr_metric_plus = (
                ...
                drum_metric_minus, non_drum_metric_minus, instr_metric_minus = (
                    compute_track_metrics(
                        ref_notes_minus,
                        notes_dict["notes"],
                        eval_vocab=self.hparams.eval_vocab[dataloader_idx],
                ...
                if max_index == 0:
                    logger.info('%s', f"ZERO: {track_info}, z/p/m: {score[0]:.2f}/{score[1]:.2f}/{score[2]:.2f}")
                elif max_index == 1:
                    # plus
                    instr_metric["onset_f_Bass"] = instr_metric_plus["onset_f_Bass"]
                    logger.info('%s', f"PLUS: {track_info}, z/p/m: {score[0]:.2f}/{score[1]:.2f}/{score[2]:.2f}")
                    write_model_output_as_midi(
                        ref_notes_plus,
                        self.hparams.write_output_dir,
                    ...
                    logger.info('%s', f"MINUS: {track_info}, z/p/m: {score[0]:.2f}/{score[1]:.2f}/{score[2]:.2f}")
                    write_model_output_as_midi(
                        ref_notes_minus,
                        self.hparams.write_output_dir,
...
def test_case_forward_mt3():
    import torch
    from amt.config.config import audio_cfg, model_cfg, shared_cfg
    from amt.model.ymt3 import YourMT3
...
def test_case_inference_mt3():
    ...
    pred_ids = model.inference(
        x, task_tokens, max_token_length=10
    )  # (2, 3, 9) (B, C, L-task_len)
...
def test_case_forward_enc_perceiver_tf_dec_t5():
    ...
    logits, loss = (
        output["logits"],
        output["loss"],
    )  # logits: (2, 1024, 596) (B, N, vocab_size)
