...
from amt.model.ops import count_parameters
...
def test_multi_channel_linear_projection():
    ...
    mclp = MultiChannelLinearProjection(
        in_ch=26, in_dim=128, out_ch=13, out_dim=256
    )  # actually nn.Identity()
    assert type(nn.Identity()) == type(mclp.linear)
    assert mclp(x).shape == (2, 13, 10, 256)  # (b, _c, t, _d)

    x = torch.randn(2, 10, 26, 128)  # (b, t, c, d)
    mclp = MultiChannelLinearProjection(
        in_ch=26, in_dim=128, out_ch=13, out_dim=512
    )  # actually nn.Identity()
...
def test_projection_layers():
    ...
    # GroupLinear
    gl = GroupLinearFlatten(
        in_features=d, flatten_out_features=target_flatten_d, num_groups=k, use_bmm=True
    )
    enc_hs_hat = gl(enc_hs)
    assert enc_hs_hat.shape == (b, t, target_flatten_d)
    logger.info('%s %s', "GroupLinear: ", f"{count_parameters(gl)//1000}k")  # 65k
    ...
    assert enc_hs_hat.shape == (b, t, target_flatten_d)
    logger.info('%s %s', "FlattenMLP: ", f"{count_parameters(fm)//1000}k")  # 3.6M

    # LinearProjection
    lp = LinearProjection(
        in_features=d, flatten_out_features=target_flatten_d, num_groups=k
    )
    enc_hs_hat = lp(enc_hs)
    assert enc_hs_hat.shape == (b, t, target_flatten_d)
    logger.info('%s %s', "LinearProjection: ", f"{count_parameters(lp)//1000}k")  # 1M

    # DepthwiseConvProjection
    dc = DepthwiseConvProjection(
        in_features=d, flatten_out_features=target_flatten_d, num_groups=k, depth=16
    )
    enc_hs_hat = dc(enc_hs)
    assert enc_hs_hat.shape == (b, t, target_flatten_d)
    logger.info('%s %s', "DepthwiseConvProjection: ", f"{count_parameters(dc)//1000}k")  # 4M
