...
from amt.config.data_presets import data_preset_multi_cfg

logger = logging.getLogger(__name__)


def get_ds(
    data_preset_multi: Dict, task_name: str, train_num_samples_per_epoch: int = 90000
):
    tm = TaskManager(task_name=task_name)
    tm.max_note_token_length_per_ch = 1024  # only to check the max length
    dm = AMTDataModule(
        data_preset_multi=data_preset_multi,
        task_manager=tm,
...
logger.info('%s %s %s', "empty_sequences:", f"{empty_sequence:.2f}", "%")

mean_except_empty = np.mean(length_all[length_all > 2])
logger.info('%s %s', "mean_except_empty:", mean_except_empty)

median_except_empty = np.median(length_all[length_all > 2])
logger.info('%s %s', "median_except_empty:", median_except_empty)

ch_less_than_768 = np.sum(length_all < 768) / len(length_all) * 100
logger.info('%s %s %s', "ch_less_than_768:", f"{ch_less_than_768:.2f}", "%")

ch_larger_than_512 = np.sum(length_all > 512) / len(length_all) * 100
logger.info('%s %s %s', "ch_larger_than_512:", f"{ch_larger_than_512:.6f}", "%")

ch_larger_than_256 = np.sum(length_all > 256) / len(length_all) * 100
logger.info('%s %s %s', "ch_larger_than_256:", f"{ch_larger_than_256:.6f}", "%")

ch_larger_than_128 = np.sum(length_all > 128) / len(length_all) * 100
logger.info('%s %s %s', "ch_larger_than_128:", f"{ch_larger_than_128:.6f}", "%")

ch_larger_than_64 = np.sum(length_all > 64) / len(length_all) * 100
logger.info('%s %s %s', "ch_larger_than_64:", f"{ch_larger_than_64:.6f}", "%")
...
logger.info('%s %s %s', "song_larger_than_512:", f"{song_larger_than_512/num_songs*100:.4f}", "%")
logger.info('%s %s %s', "song_larger_than_256:", f"{song_larger_than_256/num_songs*100:.4f}", "%")
logger.info('%s %s %s', "song_larger_than_128:", f"{song_larger_than_128/num_songs*100:.4f}", "%")
logger.info('%s %s %s', "song_larger_than_64:", f"{song_larger_than_64/num_songs*100:.4f}", "%")
