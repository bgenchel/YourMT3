...
def apply_temperature_to_attention_tensors(tensor_tuple, temperature=1.0):
    ...
    for tensor in tensor_tuple:
        ...
        flattened_tensor = tensor.reshape(1, tensor.shape[1], -1)  # Modified line here
...
def keep_top_k(matrix, k=6):
    ...
    for i in range(matrix.shape[0]):
        result_matrix[i, topk_indices_per_row[i]] = matrix[i, topk_indices_per_row[i]]
...
def test_case_forward_enc_perceiver_tf_dec_multi_t5():
    ...
    from amt.utils.data_modules import AMTDataModule
    ...
    plt.figure(figsize=(15, 10))  # Adjust these numbers as needed for width and height
    plt.subplot(2, 4, 1)
    ...
    plt.subplot(2, 4, 2)
    plt.imshow(x_conv[0][:, :, 0].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("conv(spec), ch=0")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.subplot(2, 4, 3)
    plt.imshow(x_conv[0][:, :, 42].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("ch=42")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.subplot(2, 4, 4)
    plt.imshow(x_conv[0][:, :, 80].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("ch=80")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.subplot(2, 4, 5)
    plt.imshow(x_conv[0][:, :, 11].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("ch=11")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.subplot(2, 4, 6)
    plt.imshow(x_conv[0][:, :, 20].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("ch=20")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.subplot(2, 4, 7)
    plt.imshow(x_conv[0][:, :, 77].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("ch=77")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.subplot(2, 4, 8)
    plt.imshow(x_conv[0][:, :, 90].detach().numpy().T, aspect="auto", origin="lower")
    plt.title("ch=90")
    plt.xlabel("time step")
    plt.ylabel("F")
    plt.tight_layout()
    ...
    output = model.encoder(
        inputs_embeds=x_conv, output_hidden_states=True, output_attentions=True
    )
    enc_hs_all, att, catt = (
    ...
    fig, axs = plt.subplots(
        5, 5, figsize=(10, 9)
    )  # 25 subplots arranged in 5 rows and 5 columns
    ...
    for k in range(25):  # Iterating through K indices from 0 to 24
        axs[k].imshow(
            data[:, k, :].T, cmap="viridis"
        )  # Transposing the matrix to swap T and D
    ...
    plt.subplot(311)
    plt.imshow(torch.sum(torch.sum(catt[0][0], axis=0), axis=0).detach().numpy())
    plt.title("block=0")
    plt.ylabel("latent k")
    plt.xlabel("conv channel")
    plt.subplot(312)
    plt.imshow(torch.sum(torch.sum(catt[1][0], axis=0), axis=0).detach().numpy())
    plt.title("block=1")
    plt.ylabel("latent k")
    plt.xlabel("conv channel")
    plt.subplot(313)
    plt.imshow(torch.sum(torch.sum(catt[2][0], axis=0), axis=0).detach().numpy())
    plt.title("block=2")
    plt.ylabel("latent k")
    plt.xlabel("conv channel")
    ...
    fig, axs = plt.subplots(
        3, 1, figsize=(12, 18), gridspec_kw={"height_ratios": [1, 1, 0.5]}
    )  # Adjusted for different subplot sizes
    ...
    def combined_update_smaller_att(t):
        ...
        ax_catt3.set_ylabel("latent k")
        ax_catt3.set_xlabel("conv channel")

        # Update subplot for catt with h=5
        ax_catt5.clear()
        ax_catt5.imshow(catt[2][0][t, 5, :, :].detach().numpy())
        ax_catt5.set_title(f"block=2, t={t}, head=5")
        ax_catt5.set_ylabel("latent k")
        ax_catt5.set_xlabel("conv channel")
    ...
    combined_anim_smaller_att = FuncAnimation(
        fig, combined_update_smaller_att, frames=range(0, 110), interval=200
    )
    combined_anim_smaller_att.save(
        "combined_animation_smaller_att.gif", writer="pillow", fps=5
    )

    # enc Latent Self-attention: How latent K attends to K?
    plt.subplot(231)
    plt.imshow(
        torch.sum(torch.sum(att[0][0], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B0L0")
    plt.xlabel("latent k")
    plt.ylabel("latent k")
    plt.subplot(234)
    plt.imshow(
        torch.sum(torch.sum(att[0][1], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B0L1")
    plt.xlabel("latent k")
    plt.ylabel("latent k")
    plt.subplot(232)
    plt.imshow(
        torch.sum(torch.sum(att[1][0], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B1L0")
    plt.xlabel("latent k")
    plt.ylabel("latent k")
    plt.subplot(235)
    plt.imshow(
        torch.sum(torch.sum(att[1][1], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B1L1")
    plt.xlabel("latent k")
    plt.ylabel("latent k")
    plt.subplot(233)
    plt.imshow(
        torch.sum(torch.sum(att[2][0], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B2L0")
    plt.xlabel("latent k")
    plt.ylabel("latent k")
    plt.subplot(236)
    plt.imshow(
        torch.sum(torch.sum(att[2][1], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B2L1")
    plt.xlabel("latent k")
    plt.ylabel("latent k")
    ...
    fig, axs = plt.subplots(
        len(time_steps), 8, figsize=(16, 6)
    )  # Subplots for each time step and head
    ...
    plt.subplot(231)
    plt.imshow(
        torch.sum(torch.sum(att[0][2], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B0L2")
    plt.xlabel("t")
    plt.ylabel("t")
    plt.subplot(234)
    plt.imshow(
        torch.sum(torch.sum(att[0][3], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B0L3")
    plt.xlabel("t")
    plt.ylabel("t")
    plt.subplot(232)
    plt.imshow(
        torch.sum(torch.sum(att[1][2], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B1L2")
    plt.xlabel("t")
    plt.ylabel("t")
    plt.subplot(235)
    plt.imshow(
        torch.sum(torch.sum(att[1][3], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B1L3")
    plt.xlabel("t")
    plt.ylabel("t")
    plt.subplot(233)
    plt.imshow(
        torch.sum(torch.sum(att[2][2], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B2L2")
    plt.xlabel("t")
    plt.ylabel("t")
    plt.subplot(236)
    plt.imshow(
        torch.sum(torch.sum(att[2][3], axis=1), axis=0).detach().numpy(), origin="upper"
    )
    plt.title("B2L3")
    plt.xlabel("t")
    plt.ylabel("t")
    ...
    fig, axs = plt.subplots(
        13, 6, figsize=(21, 39)
    )  # 13 rows (for k=0:12) and 7 columns (for head=0:6)
    for k in range(13):
        s = seq_lengths[k]
        for head in range(6):
    ...
    k = 10
    logger.info('%s', torch.argmax(logits, dim=3)[0, k, :])
