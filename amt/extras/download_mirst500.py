...
def downloadMp3(yt, idx, askPath=0):
    ...
    if (askPath == 1):
        logger.info('%s', "Enter the destination (leave blank for default dir mp3File)")
    ...
    # result of success
    logger.info('%s', yt.title + " has been successfully downloaded.")
...
for i in MISSING_FILE_IDS:
    logger.info('%s', f'Downloading {i}...')
    yt = YouTube(data_link[str(i)])
    try:
        downloadMp3(yt, idx=i)
    except:
        download_fail.append(i)
        logger.info('%s', f'Failed to download {i}.')

logger.info('%s', f'Failed to download {len(download_fail)} files: {download_fail}')