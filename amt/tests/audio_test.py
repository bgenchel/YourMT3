...
import wave
import tempfile
from amt.utils.audio import load_audio_file
from amt.utils.audio import get_audio_file_info
from amt.utils.audio import slice_padded_array
from amt.utils.audio import slice_padded_array_for_subbatch
from amt.utils.audio import write_wav_file
...
class TestLoadAudioFile(unittest.TestCase):
    ...
    def test_load_audio_file(self):
        ...
        audio_data = load_audio_file(
            temp_filename, seg_start_sec, seg_length_sec, dtype=np.int16
        )
    ...
    def test_content(self):
        ...
        for i in range(sliced_x.shape[0] - 1):
            np.testing.assert_array_equal(
                sliced_x[i, :], self.x[:, i * 50 : i * 50 + 100].flatten()
            )
        ...
        np.testing.assert_array_equal(
            last_slice[: len(last_slice_no_padding)], last_slice_no_padding
        )
...
class TestSlicePadForSubbatch(unittest.TestCase):
    ...
    def test_slice_padded_array_for_subbatch(self):
        ...
        result = slice_padded_array_for_subbatch(
            input_array, slice_length, slice_hop, pad, sub_batch_size
        )
