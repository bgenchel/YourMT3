...
class TestAMTMetrics(unittest.TestCase):
    ...
    def test_bulk_update_and_compute(self):
        ...
        d2 = {
            "onset_f": {"value": 0.5, "weight": 1.0},
            "offset_f": {"value": 0.5, "weight": 1.0},
        }
    ...
    def test_compute_track_metrics_singing(self):
        from amt.config.vocabulary import SINGING_SOLO_CLASS, GM_INSTR_CLASS_PLUS
        from amt.utils.event2note import note_event2note

        ref_notes_dict = np.load(
            "extras/examples/singing_notes.npy", allow_pickle=True
        ).tolist()
        ref_note_events_dict = np.load(
            "extras/examples/singing_note_events.npy", allow_pickle=True
        ).tolist()
        est_notes, _ = note_event2note(ref_note_events_dict["note_events"])
        ref_notes = ref_notes_dict["notes"]

        metric = AMTMetrics(
            prefix=f"test/", extra_classes=[k for k in SINGING_SOLO_CLASS.keys()]
        )
        ...
        metric = AMTMetrics(
            prefix=f"test/", extra_classes=[k for k in GM_INSTR_CLASS_PLUS.keys()]
        )
        drum_metric, non_drum_metric, instr_metric = compute_track_metrics(
            est_notes,
            ref_notes,
