...
precision = (
    "16"  # if torch.cuda.is_available() else '32'# @param ["32", "bf16-mixed", "16"]
)
project = "2024"
...
def prepare_media(
    source_path_or_url: os.PathLike,
        ...
        with open(log_file, "w") as lf:
            ...
            for line in iter(process.stdout.readline, ""):
                ...
                if "www.google.com/device" in line:
                    hl_text = line.replace(
                        "https://www.google.com/device",
                        "\033[93mhttps://www.google.com/device\x1b[0m",
                    ).split()
...
with gr.Blocks(theme=theme, css=css) as demo:
    ...
    with gr.Group():
        ...
        with gr.Tab("From YouTube"):
            with gr.Column(scale=4):
                ...
                youtube_url = gr.Textbox(
                    label="YouTube Link URL", placeholder="https://youtu.be/..."
                )
                # Display examples
                gr.Examples(examples=YOUTUBE_EXAMPLES, inputs=youtube_url)
                # Play button
                play_video_button = gr.Button(
                    "Get Audio from YouTube", variant="primary"
                )
                # Play youtube
            ...
            with gr.Column(scale=4):
                with gr.Row():
                    ...
                    # Oauth button
                    oauth_button = gr.Button(
                        "google.com/device",
                        variant="primary",
                        link="https://www.google.com/device",
                    )
            ...
            with gr.Column(scale=1):
                ...
                transcribe_video_button.click(
                    process_audio_yt_temp, inputs=youtube_url, outputs=output_tab2
                )
                # transcribe_video_button.click(process_video, inputs=youtube_url, outputs=output_tab2)
                # Play
                play_video_button.click(
                    play_video, inputs=youtube_url, outputs=youtube_player
                )
            with gr.Column(scale=1):
        ...
        with gr.Tab("Upload audio"):
            ...
            # Transcribe
            output_tab1 = gr.HTML()
            transcribe_audio_button.click(
                process_audio, inputs=audio_input, outputs=output_tab1
            )
